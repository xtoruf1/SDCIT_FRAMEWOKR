...
<input type="hidden" id="topMenuId" name="topMenuId" value="0" />
				...
				<td>
					<select id="searchCondition" name="searchCondition" class="form_select">
						...
						<c:forEach var="top" items="${topList}" varStatus="status">
							<option value="${top.topMenuId}" <c:if test="${param.searchCondition eq top.topMenuId}">selected="selected"</c:if>>${top.topMenuName}</option>
						</c:forEach>
					</select>
				</td>
            </tr>
...
<script type="text/javascript">
	...
	ibHeader.addHeader({Header: '대메뉴', Type: 'Html', SaveName: 'topMenuName', Width: 70, Align: 'Left'});
	ibHeader.addHeader({Header: 'Level2', Type: 'Html', SaveName: 'lev1Name', Width: 70, Align: 'Left'});
	ibHeader.addHeader({Header: 'Level3', Type: 'Html', SaveName: 'lev2Name', Width: 70, Align: 'Left'});
	ibHeader.addHeader({Header: 'Tab', Type: 'Html', SaveName: 'lev3Name', Width: 70, Align: 'Left'});
	<c:forEach var="auth" items="${authList}" varStatus="status">
		ibHeader.addHeader({Header: '${auth.authName}', Type: 'CheckBox', SaveName: 'auth${auth.authId}', Width: 30, Align: 'Center', HeaderCheck: 0, TrueValue: 'Y', FalseValue: 'N'});
	</c:forEach>
	...
	function getList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/menu/user/selectList.do" />'
	...
	function menuListSheet_OnLoadData(data) {
		...
		rowdata.forEach(function(item, index){
			...
			menuNm += item.topMenuName;
			menuNm += '	</div>';
			if (topMenuId != item.topMenuId) {
				menuNm += '	<div style="margin-bottom: 4px;margin-right: 5px;">';
				menuNm += '		<button type="button" onclick="showProgramList(\'\', \'' + item.topMenuId + '\', \'' + item.topMenuName + '\');" style="cursor: pointer;width: 16px;height: 16px;background: url(<c:url value="/images/icon/icon_plus.png" />) no-repeat center;border-radius: 2px;border: 1px solid #bbb;"><span class="blind"></span></button>';
				menuNm += '	</div>';
			}
			menuNm += '</div>';

			newObj.Data[index].topMenuName = menuNm;
			...
			if (item.menuDepth != 3) {
				menuNm += '	<div style="margin-bottom: 4px;margin-right: 5px;">';
				menuNm += '		<button type="button" onclick="showProgramList(\'' + nvlString(item.menuSetId, '') + '\', \'' + item.topMenuId + '\', \'' + nvlString(levName, '') + '\');" style="cursor: pointer;width: 16px;height: 16px;background: url(<c:url value="/images/icon/icon_plus.png" />) no-repeat center;border-radius: 2px;border: 1px solid #bbb;"><span class="blind"></span></button>';
				menuNm += '	</div>';
			}
			menuNm += '</div>';
	...
	function menuListSheet_OnRowSearchEnd(row) {
	    ...
	    if (isStringEmpty(menuListSheet.GetCellValue(row, 'menuSetId'))) {
	    	...
	    	<c:forEach var="auth" items="${authList}" varStatus="status">
				menuListSheet.SetCellEditable(row, 'auth${auth.authId}', 0);
				// 편집시 컬럼 색깔 변경
				menuListSheet.SetCellBackColor(row, 'auth${auth.authId}', '#ffffff');
			</c:forEach>
	    }
	}
	...
	function menuListSheet_OnChange(row, col, value) {
		if (row > 0) {
			...
			// 권한을 클릭시 상위 메뉴가 다 클릭되도록 하위메뉴가 다 풀리도록 한다.
			// SetCellValue 에서 Onchange 이벤트가 자동적으로 걸리므로 재귀를 자동적으로 해준다.
			<c:forEach var="auth" items="${authList}" varStatus="status">
				if (saveName == 'auth${auth.authId}') {
					var changePgmId = menuListSheet.GetCellValue(row, 'pgmId');
					var changeUpperId = menuListSheet.GetCellValue(row, 'upperId');

					for (var i = 0; i < menuListSheet.LastRow(); i++) {
						var pgmId = menuListSheet.GetCellValue(i, 'pgmId');
						var upperId = menuListSheet.GetCellValue(i, 'upperId');

						if (pgmId == changeUpperId) {
							if (value == 'Y') {
								menuListSheet.SetCellValue(i, 'auth${auth.authId}', value);
							}
						}

						if (upperId == changePgmId) {
							if (value == 'N') {
								menuListSheet.SetCellValue(i, 'auth${auth.authId}', value);
							}
						}
	...
	function showProgramList(menuSetId, topMenuId, menuName) {
		f.menuSetId.value = menuSetId;
		f.topMenuId.value = topMenuId;

		global.openLayerPopup({
			popupUrl : '<c:url value="/menu/user/popup/programList.do" />'
	...
	function doInsertMenuProgram(pgmId) {
		f.pgmId.value = pgmId;

		global.ajax({
			type : 'POST'
			, url : '<c:url value="/menu/user/insertMenuProgram.do" />'
			, data : $('#menuForm').serialize()
			, dataType : 'json'
			, async : true
			, spinner : true
	...
	function showMenuList(menuSetId, upperId) {
		global.openLayerPopup({
			popupUrl : '<c:url value="/menu/user/popup/menuList.do" />'
			, params : {
				menuSetId : menuSetId
				, upperId : upperId
			}
			, callbackFunction : function(resultObj){
				if (confirm('메뉴 정렬순서를 변경 하시겠습니까?')) {
					doUpdateMenuSort(resultObj);
				}
			}
		});
	}

	function doUpdateMenuSort(menuSetIds) {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/menu/user/updateMenuSort.do" />'
			, data : {
				menuSetIds : menuSetIds
			}
			, dataType : 'json'
			, async : true
			, spinner : true
			...
			if (confirm('저장 하시겠습니까?')) {
				...
				$.each(saveJson, function(key1, value1) {
					$.each(value1, function(key2, value2) {
						...
						<c:forEach var="auth" items="${authList}" varStatus="status">
							map.auth${auth.authId} = value2.auth${auth.authId};
						</c:forEach>
				...
				global.ajax({
					type : 'POST'
					, url : '<c:url value="/menu/user/updateMenu.do" />'
