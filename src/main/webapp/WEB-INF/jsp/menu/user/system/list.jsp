	...
	<table class="formTable">
		...
		<tbody>
			<tr>
				<th scope="row">검색어</th>
				<td>
					<input type="text" id="searchKeyword" name="searchKeyword" value="${param.searchKeyword}" onkeydown="onEnter(doSearch);" class="form_text w100p" />
				</td>
            </tr>
		</tbody>
	</table>
...
<script type="text/javascript">
	...
	ibHeader.addHeader({Header: '대메뉴명', Type: 'Text', SaveName: 'topMenuName', Width: 220, Align: 'Left', Edit: 0, Cursor: 'Pointer'});
	ibHeader.addHeader({Header: '순번', Type: 'Text', SaveName: 'sortSeq', Width: 50, Align: 'Center', Edit: 0});
	ibHeader.addHeader({Header: '등록일시', Type: 'Text', SaveName: 'creDate', Width: 70, Align: 'Center', Edit: 0});
	
	ibHeader.addHeader({Header: '대메뉴아이디', Type: 'Text', SaveName: 'topMenuId', Hidden: true});
	...
	function systemListSheet_OnSearchEnd(code, msg) {
		if (code != 0) {
    		...
    		// 대메뉴명에 볼드 처리
			systemListSheet.SetColFontBold('topMenuName', 1);
    	}
    }
	
	function systemListSheet_OnClick(row, col, value, cellX, cellY, cellW, cellH) {
		if (row > 0) {
			if (systemListSheet.ColSaveName(col) == 'topMenuName') {
				var topMenuId = systemListSheet.GetCellValue(row, 'topMenuId');
	...
	function getList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/menu/user/system/selectList.do" />'
	...
	function systemListSheet_OnRowSearchEnd(row) {
	    ...
	    if (systemListSheet.GetCellValue(row, 'settingYn') == 'Y') {
	    	...
	    	systemListSheet.SetToolTipText(row, 'delFlag', '메뉴에 등록된 대메뉴는 삭제할 수 없습니다.');
	    } 
	}
	
	// 등록화면
	function goWrite() {
		f.action = '<c:url value="/menu/user/system/regist.do" />';
		f.topMenuId.value = '0';
		f.target = '_self';
		f.submit();
	}
	...
	// 조회화면
	function goView(topMenuId) {
		f.action = '<c:url value="/menu/user/system/modify.do" />';
	...
	function doDelete() {		
		...
		if (saveJson.data.length) {
			if (confirm('삭제 하시겠습니까?')) {
				...
				global.ajax({
					type : 'POST'
					, url : '<c:url value="/menu/user/system/deleteList.do" />'
					...
					, spinner : true
					, success : function(data){
						location.href = '<c:url value="/menu/user/system/list.do" />';
					}
				});
			}
		} else {
			alert('삭제할 대메뉴 정보를 선택해 주세요.');
		}
	}
</script>