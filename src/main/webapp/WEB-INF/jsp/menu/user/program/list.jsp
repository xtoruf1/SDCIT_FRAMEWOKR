...
<script type="text/javascript">
	...
	function getList() {
		...
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/menu/user/program/selectList.do" />'
	...
	// 등록화면
	function goWrite() {
		f.action = '<c:url value="/menu/user/program/regist.do" />';
		f.pgmId.value = '0';
		f.target = '_self';
		f.submit();
	}
	...
	function goView(pgmId) {
		f.action = '<c:url value="/menu/user/program/modify.do" />';
		f.pgmId.value = pgmId;
		f.target = '_self';
		f.submit();
	}
	...
	function doDelete() {
		...
		if (saveJson.data.length) {
			if (confirm('삭제 하시겠습니까?')) {
				...
				global.ajax({
					type : 'POST'
					, url : '<c:url value="/menu/user/program/deleteList.do" />'
					...
					, spinner : true
					, success : function(data){
						location.href = '<c:url value="/menu/user/program/list.do" />';
					}
				});
			}
		} else {
