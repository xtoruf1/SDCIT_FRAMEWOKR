            ...
            <tr>
				...
				<td colspan="3">
					<label class="label_form">
						<input type="radio" class="form_radio" name="showDiv" value="1" checked="checked">
...
<script type="text/javascript">
	...
	function initIBSheet(){
		...
		ibHeader.setConfig({AutoFitColWidth: 'colhidden', Page: 50, SearchMode: 2, DeferredVScroll: 1, SizeMode: 4, MouseHoverMode: 2, NoFocusMode : 0, MergeSheet: 1});
		...
		sheet1.ShowSubSum([{StdCol:"tradeDept", SumCols:"5|22", Sort:true, ShowCumulate:false, CaptionCol:0, Align:"Right", OtherColText:"tradeDept=소    계;coNmKor=|cnt|"}]);
	...
	function getList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/tfms/ssm/selectRecommendedSpeList.do" />'
	...
	function getAssuranceList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/tfms/ssm/selectRecommendedSpeAssuranceList.do" />'
			, data : $('#form1').serialize()
			, dataType : 'json'
			, async : true
			, spinner : true
	...
	function getBankList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/tfms/ssm/selectRecommendedSpeBankList.do" />'
			, data : $('#form1').serialize()
			, dataType : 'json'
			, async : true
			, spinner : true
	...
	function sheet1_OnSearchEnd(code, msg) {
		sheet1.SetCellAlign(sheet1.LastRow()-1, 'coNmKor', "Right");
	...
	function sheet2_OnSearchEnd(code, msg) {
		sheet2.SetCellAlign(sheet2.LastRow()-1, 'coNmKor', "Right");
	...
	function sheet3_OnSearchEnd(code, msg) {
		sheet3.SetCellAlign(sheet3.LastRow()-1, 'coNmKor', "Right");
