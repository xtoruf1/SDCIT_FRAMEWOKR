...
// 		ibHeader.setConfig({AutoFitColWidth: 'search|resize|init|colhidden|rowtransaction|colresize', Page: 10, SearchMode: 4, DeferredVScroll: 1, SizeMode: 1, MouseHoverMode: 2, NoFocusMode : 0});
	...
	function getList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/tfms/ssm/selectApplicationRecommendationSpeSendList.do" />'
...
/*
	...
	function doSendFax(){
		...
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/tfms/ssm/applicationRecommendationSpeSendFaxList.do" />'
	...
	function doSendMms(){
		...
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/tfms/ssm/applicationRecommendationSpeSendMmsList.do" />'
			, data : JSON.stringify(ccf)
			, contentType : 'application/json'
			, dataType : 'json'
			, async: false
	...
	function doSendMail(){
		...
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/tfms/ssm/applicationRecommendationSpeSendMailList.do" />'
			, data : JSON.stringify(ccf)
			, contentType : 'application/json'
			, dataType : 'json'
			, async: false
	...
	function doExcel(){
        ...
        if(sheet1.RowCount() > 0){
        	...
        	downloadIbSheetExcel(sheet1, "추천서발송(특별)", "");
	...
	function doDeadine(){
		...
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/tfms/ssm/applicationRecommendationSpeSendDeadine.do" />'
			, data : $('#form1').serialize()
			, dataType : 'json'
			, async : true
			, spinner : true
	...
	function doSendPopup(Row){
		...
		global.openLayerPopup({
			popupUrl : '<c:url value="/tfms/ssm/applicationRecommendationSpeSendPopup.do" />'	//tfms/lms/FundRegisteDetail.screen
			, params : {
			}
			, callbackFunction : function(resultObj){
			}
