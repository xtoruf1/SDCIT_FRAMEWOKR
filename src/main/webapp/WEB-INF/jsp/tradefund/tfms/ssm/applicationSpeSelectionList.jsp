...
<%--
	<table class="formTable">
		...
		<tr>
			<th>기금융자명</th>
			<td colspan="5"><c:out value="${fnd0000tVo.title}"/></td>
		</tr>
		<tr>
			<th>접수기간</th>
			<td><c:out value="${fnd0000tVo.bsnDt}"/></td>
			<th>융자 1차 지급일</th>
			<td><c:out value="${fnd0000tVo.fund1Dt}"/></td>
			<th>추천유효기간</th>
			<td><c:out value="${fnd0000tVo.validDt}"/></td>
		</tr>
		<tr>
			<th>사업기간</th>
			<td><c:out value="${fnd0000tVo.bsnAplDt}"/></td>
			<th>융자 2차 지급일</th>
			<td><c:out value="${fnd0000tVo.fund2Dt}"/></td>
			<th>진행상태</th>
			<td><c:out value="${fnd0000tVo.stNm}"/></td>
		</tr>
	 </table>
...
<script type="text/javascript">
	...
	function getList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/tfms/ssm/selectApplicationSpeSelectionList.do" />'
	...
	function doExcel(){
        ...
        if(sheet1.RowCount() > 0){
        	downloadIbSheetExcel(sheet1, '신청업체선정(특별)', '');
        }else{
	...
	function goApplicationSelect(Row){
		...
		var url = "";
		url = '<c:url value="/tfms/ssm/applicationSpeSelection.do" />';
