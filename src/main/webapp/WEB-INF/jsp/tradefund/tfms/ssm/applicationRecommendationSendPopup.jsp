			...
			<tbody>
				<tr>
					...
					<td>
						...
						<label class="label_form">
							<input type="checkbox" class="form_radio sendType" id="sendAlimtalk" name="sendAlimtalk" value="Y" onclick="chkSend()" >
							<span class="label">알림톡</span>
						</label>
						<label class="label_form">
							<input type="checkbox" class="form_radio sendType" id="sendFax" name="sendFax" value="Y" onclick="chkSend()">
							<span class="label">팩스</span>
						</label>
						<label class="label_form">
							<input type="checkbox" class="form_radio sendType" id="sendEamil" name="sendEamil" value="Y" onclick="chkSend()">
...
<script type="text/javascript">
	...
	function chkSend(){
		var chkSendAlimtalk =  $("#sendAlimtalk").is(":checked");
		var chkSendFax = $("#sendFax").is(":checked");
		var chkSendEamil = $("#sendEamil").is(":checked");

		if(chkSendAlimtalk && chkSendFax && chkSendEamil){ //대상이 체크 되어 있을 때
	    	//모든 체크박스 체크
	        $("#sendAll").prop("checked", true);
	    }else{ //대상이 체크 해제 되어 있을 때
	    	//모든 체크박스 체크해제
...
// 		}
		...
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/tfms/ssm/applicationRecommendationSendPopupTotal.do" />'
