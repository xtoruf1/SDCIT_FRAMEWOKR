...
<script type="text/javascript">
	...
	function initIBSheet(){
        ...
        ibHeader.addHeader({Header:"기금융자명",  	Type:"Text",      Hidden:false,  Width:270,  Align:"Left",    SaveName:"title",         CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:false,   InsertEdit:true,   EditLen:30, Ellipsis:true  });
	...
	function getList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/tfms/ssm/selectApplicationList.do" />'
	...
	function doExcel(){
        ...
        if(sheet1.RowCount() > 0){
        	downloadIbSheetExcel(sheet1, "신청업체조회", "");
        }else{
	...
	function goApplicationSelect(Row){
		...
		var url = '<c:url value="/tfms/ssm/applicationDetail.do" />';
		f.action = url;
		f.target = "_self";
		f.submit();
	}
