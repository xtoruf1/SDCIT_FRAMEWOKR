			...
			<tr>
				...
				<td>
					<fieldset class="widget">
						<select id="searchFndFindt" name="searchFndFindt" class="form_select" title="기금융자년도" style="width: 130px;">
							<c:forEach var="yearMonth" items="${yearMonthList}" varStatus="status">
								<option value="<c:out value="${yearMonth.fndYdt}"/>" <c:if test="${yearMonth.fndYdt eq yearMonthChk}">selected="selected"</c:if>><c:out value="${yearMonth.fndYdt}"/></option>
							</c:forEach>
						</select>
					</fieldset>
				</td>
...
<script type="text/javascript">
	...
	function initIBSheet(){
		...
		ibHeader.addHeader({Header:"총업체수",    	Type:"AutoSum",   Hidden:false, Width:100,  Align:"Center",  ColMerge:false,  SaveName:"fndCntSum",    CalcLogic:"",   Format:"Integer",     PointCount:0,   UpdateEdit:false,   InsertEdit:true,   EditLen:30 });
		ibHeader.addHeader({Header:"업체수",     	Type:"AutoSum",   Hidden:false, Width:100,  Align:"Center",  ColMerge:false,  SaveName:"fndCntSum2",   CalcLogic:"",   Format:"Integer",     PointCount:0,   UpdateEdit:false,   InsertEdit:true,   EditLen:30 });
		...
		ibHeader.addHeader({Header:"예상상환금액", 	Type:"AutoSum",   Hidden:false, Width:150,  Align:"Right",   ColMerge:false,  SaveName:"fndYamtSum",   CalcLogic:"",   Format:"Integer",     PointCount:0,   UpdateEdit:false,   InsertEdit:true,   EditLen:30 });
	...
	function initDetailIBSheet(){
		...
		ibHeader.addHeader({Header:"No",       	Type:"Seq",       Hidden:false,  Width:60,   Align:"Center",  ColMerge:true,  SaveName:"no"             });
		ibHeader.addHeader({Header:"chk",       Type:"Text",      Hidden:true,   Width:60,   Align:"Center",  ColMerge:true,  SaveName:"chk"           });
        ...
        ibHeader.addHeader({Header:"대표자명",    	Type:"Text",      Hidden:false,  Width:100,  Align:"Center",  ColMerge:false, SaveName:"ceoNmKor",      CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:false,   InsertEdit:true,   EditLen:30 });
        ...
        ibHeader.addHeader({Header:"예상상환금액", 	Type:"AutoSum",   Hidden:false,  Width:120,  Align:"Right",   ColMerge:false, SaveName:"fndYamtSum",    CalcLogic:"",   Format:"Integer",     PointCount:0,   UpdateEdit:false,   InsertEdit:true,   EditLen:30 });
	...
	function doClear(){
		...
		setSelect(f.searchFndFindt, '<c:out value="${yearMonthChk}"/>');
	}
	...
	function getList() {
		...
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/tfms/rpm/selectPrepaymentHistoryFirstList.do" />'
	...
	function getDetailList() {
		...
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/tfms/rpm/selectPrepaymentHistoryDetailList.do" />'
			, data : $('#form1').serialize()
			, dataType : 'json'
			, async : true
			, spinner : true
