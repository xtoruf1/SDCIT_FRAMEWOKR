	...
	<table class="formTable">
		...
		<tbody>
			<tr>
				...
				<th>구분</th>
				<td>
					<select id="acctTypeId" name="acctTypeId" class="form_select">
						<option value="">전체</option>
						<c:forEach var="list" items="${com000}" varStatus="status">
							<option value="${list.code}" <c:if test="${param.acctTypeId eq list.code}">selected="selected"</c:if>>${list.codenm}</option>
						</c:forEach>
					</select>
				</td>
            </tr>
...
<script type="text/javascript">
	...
	ibHeader.addHeader({Header: '코드구분', Type: 'Text', SaveName: 'acctTypeNm', Width: 30, Align: 'Center', Edit: 0});
	ibHeader.addHeader({Header: '코드', Type: 'Text', SaveName: 'basecd', Width: 50, Align: 'Center', Edit: 0, Cursor: 'Pointer'});
	ibHeader.addHeader({Header: '코드명', Type: 'Text', SaveName: 'basenm', Width: 50, Align: 'Left', Edit: 0});
	...
	function getList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/tmscommon/code/selectSycsCodeList.do" />'
	...
	// 등록화면
	function goWrite() {
		f.action = '<c:url value="/tmscommon/code/regist.do" />';
		f.basecd.value = '';
		f.target = '_self';
		f.submit();
	}
	...
	function goView(basecd) {
		f.action = '<c:url value="/tmscommon/code/modify.do" />';
	...
	function doDelete() {
		...
		if (saveJson.data.length) {
			if (confirm('삭제 하시겠습니까?')) {
				...
				global.ajax({
					type : 'POST'
					, url : '<c:url value="/tmscommon/code/deleteList.do" />'
					...
					, success : function(data){
						if (data.result) {
							location.href = '<c:url value="/tmscommon/code/list.do" />';
						} else {
							alert(data.message);
						}
					}
