	...
	<div class="foldingTable fold">
		...
		<%-- 							<button type="button" onclick="clearPickerValue('searchLoanEd');" class="dateClear"><img src="<c:url value='/images/admin/btn_clear.png' />" alt="초기화" /></button> --%>
		            ...
		            <tr>
						...
						<td colspan="3">
							<fieldset class="widget">
								<select id="searchTradeDept" name="searchTradeDept" class="form_select"  <c:if test='${user.awardAuthType ne "ADMIN"}'>disabled="disabled"</c:if>  >
									<c:if test='${user.awardAuthType eq "ADMIN"}'>
									<option value="" >::: 전체 :::</option>
									</c:if>

									<c:forEach var="item" items="${COM001}" varStatus="status">
									<c:if test='${user.awardAuthType ne "ADMIN"}'>
										<c:if test="${item.chgCode01 eq user.awardDeptCd}">
...
<script type="text/javascript">
	...
	function getList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/sycs/send/selectSendLogSearchAward.do" />'
