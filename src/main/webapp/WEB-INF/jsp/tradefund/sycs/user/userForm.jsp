...
<input type="hidden" id="userSabun" name="userSabun" value="" />
<!-- 페이지 위치 -->
...
<div class="cont_block">
	...
	<table class="formTable">
		...
		<tr>
			...
			<td>
				<input type="text" class="form_text w100p" id="userName" name="userName" value="" maxlength="20" required="required"  title="성명" />
			</td>
		</tr>
		<tr>
			<th>전화번호<strong class="point">*</strong></th>
			<td>
				<input type="text" class="form_text w100p" id="telNo" name="telNo" value="" maxlength="20" required="required"  title="전화번호" />
			</td>
			<th>핸드폰<strong class="point">*</strong></th>
			<td>
				<input type="text" class="form_text w100p" id="handTelNo" name="handTelNo" value="" maxlength="20" required="required"  title="핸드폰" />
			</td>
		</tr>
		<tr>
			<th>팩스번호</th>
			<td>
				<input type="text" class="form_text w100p" id="faxNo" name="faxNo" value="" maxlength="20"  title="팩스번호" />
			</td>
			<th>이메일</th>
			<td>
				<input type="text" class="form_text w100p" id="email" name="email" value="" maxlength="20"  title="이메일" />
			</td>
		</tr>
		<tr>
				...
				<div class="form_row" style="width: 300px;">
					<select id="acctTypeId" name="acctTypeId" class="form_select" onchange="getAuth(this.value);">
						<c:forEach var="item" items="${acctTypeUseList}" varStatus="status">
							<option value="<c:out value="${item.acctTypeId}"/>" ><c:out value="${item.acctTypeName}"/></option>
						</c:forEach>
					</select>
	...
	function showUserList() {
		global.openLayerPopup({
			...
			, callbackFunction : function(resultObj){
				...
				$('#userSabun').val(resultObj.userId);
				$('#userName').val(resultObj.userNm);
			}
		});
	}
</script>