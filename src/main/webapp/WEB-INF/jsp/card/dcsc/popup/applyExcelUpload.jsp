...
<form id="form" name="form" action="" method="post">
	...
	<div class="btn_group mt-20 _center">
		<button type="button" onclick="doExcelUpload();" class="btn btn_primary">저장</a>
	</div>
</form>
		...
		if (uploadfile_check(f2)) {
			if (confirm('업로드 하시겠습니까?')) {
				$('#form').ajaxSubmit({
					...
					, type : 'POST'
					, dataType : 'json'
					, url : '/card/dcsc/saveExcelUpload.do'
					, success : function(data) {
						if (data.status == '') {
			            	...
			            	if (data.status == '0000') {
			            		...
			            		closeLayerPopup();
			            	} else if (data.status.split('&')[0]  == '9010') {
			            		alert(excelRow + "번째 Row에 " + excelCell + "번째 Cell의 " + '"' + excelCellName + '"' + "(신청,접수,완료)를 입력하세요.");
