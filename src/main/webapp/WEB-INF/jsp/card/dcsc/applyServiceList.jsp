...
<div class="location">
	...
	<div class="ml-15">
		...
		<button type="button" class="btn_sm btn_primary" onclick="getDcscApplyList();">검색</button>
	</div>
</div>
			...
			<table class="formTable">
				...
				<tbody>
					<tr>
						...
						<td>
							<select id="searchCategoryId" name="searchCategoryId" class="form_select">
								...
								<c:forEach var="list" items="${categoryComboList}" varStatus="status">
						...
						<th>제휴업체</th>
						<td>
							<select id="searchAffiliateId" name="searchAffiliateId" class="form_select">
								<option value="">전체</option>
								<c:forEach var="list" items="${affiliateComboList}" varStatus="status">
									<option value="${list.code}">${list.codeNm}</option>
								</c:forEach>
							</select>
						</td>
					...
					<tr>
						<th>업체명</th>
						<td>
							<input type="text" id="searchCompanyName" name="searchCompanyName" class="form_text" onkeydown="onEnter(getDcscApplyList);" maxlength="70" />
						</td>
						<th>사업자번호</th>
						<td>
							<input type="text" id="searchBusinessNo" name="searchBusinessNo" class="form_text" onkeydown="onEnter(getDcscApplyList);return onlyNumber(event);" onkeyup="removeChar(event);" maxlength="10" />
						</td>
						<th>무역업번호</th>
						<td>
							<input type="text" id="searchTradeNo" name="searchTradeNo" class="form_text" onkeydown="onEnter(getDcscApplyList);return onlyNumber(event);" onkeyup="removeChar(event);" maxlength="8" />
						</td>
					</tr>
					<tr>
						<th>신청자</th>
						<td>
							<input type="text" id="searchApplyName" name="searchApplyName" class="form_text" onkeydown="onEnter(getDcscApplyList);" maxlength="30" />
						</td>
						<th>신청일</th>
						<td colspan="3">
	...
	<div class="cont_block">
		<div class="tbl_opt">
			...
			<div class="btnGroup ml-auto">
				...
				<select id="pageUnit" name="pageUnit" onchange="getDcscApplyList();" class="form_select ml-auto" title="목록수">
...
<script type="text/javascript">
	...
	function setSheetHeader_dcscApplySheet() {
		...
		ibHeader.addHeader({Header: '제휴업체'			, Type: 'Text'			, SaveName: 'affiliateName'		, UpdateEdit: 0, Width: 50		, Align: 'Center', BackColor: '#F6F6F6' });
		ibHeader.addHeader({Header: '신청업체'			, Type: 'Text'			, SaveName: 'companyName'		, UpdateEdit: 0	, Width: 50		, Align: 'Center', BackColor: '#F6F6F6' });
	...
	// 카테고리 목록 조회
	function getDcscApplyList() {
		if(checkInputLength($('#searchCompanyName').val(), 70, '업체명은 70자 이하로 입력해 주세요.')) return false;
		if(checkInputLength($('#searchApplyName').val(), 30, '신청자는 30자 이하로 입력해 주세요.')) return false;

		var searchParams = $('#searchForm').serializeObject();

		global.ajax({
			type : 'POST'
			, url : '/card/dcsc/selectDcscApplyList.do'
	...
	function doExcelDownload() {
		$('#searchForm').attr('action', '/card/dcsc/applyExcelDownload.do');
		$('#searchForm').submit();
	}
	...
	function applySheet_OnLoadData(data) {
		...
		rowdata.forEach(function(item, index){
			...
			var html = '<button type="button" onclick="openApplyDetailPopup(' + applyId + ');" class="btn_tbl_primary" style="cursor: pointer; width: 90%;">상세</button>';
	...
	function openApplyDetailPopup(applyId) {
		global.openLayerPopup({
			popupUrl : '/card/dcsc/applyServiceDetailPopup.do?applyId=' + applyId
		});
	}
</script>