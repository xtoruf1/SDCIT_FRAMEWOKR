...
<div class="cont_block">
	...
	<div class="tabGroup">
		<div class="tab_header">
			<button class="tab on" onclick="fn_list();">현재배너</button>
			<button class="tab"    onclick="fn_Closelist();">지난배너</button>
		</div>
		<form id="bannerRegForm" name="bannerRegForm" action="" method="post">
			<input type="hidden" name="subBannerId" id="subBannerId" value='<c:out value="${resultData.subBannerId}"/>'/>
			<table class="formTable">
				...
				<tr>
					<th>유튜브 링크</th>
					<td>
						https://www.youtube.com/embed/<input type="text" class="form_text" name="youtubeUrl" id="youtubeUrl" maxlength="120" value='<c:out value="${resultData.youtubeUrl}"/>' style="width: 120px;" />&nbsp;&nbsp;&nbsp;&nbsp;<button type="button" id="btnHelp" class="btn_tbl" onclick="helpPop();">HELP</button>
					</td>
				</tr>
				<tr>
					<th>공개여부</th>
					<td>
						<label class="label_form">
							<input type="radio" name="viewYn" class="form_radio" id="viewYnY" value="Y" <c:out value="${resultData.viewYn eq 'Y' ? 'checked' : ''}"/>>
							<span class="label">공개</span>
						</label>
						<label class="label_form">
							<input type="radio" name="viewYn" class="form_radio" id="viewYnN" value="N" <c:out value="${resultData.viewYn eq 'N' ? 'checked' : ''}"/>>
...
<script type="text/javaScript" language="javascript">
	...
	function goList(){
		location.href = '<c:out value="${listUrl}"/>';
	}
	...
	function modifyBanner() {
		...
		if(confirm('수정 하시겠습니까?')){
			global.ajax({
				type : 'POST'
				, url : '<c:url value="/mainContent/modifySubBanner.do" />'
	...
	function deleteBanner() {
		if(confirm('삭제 하시겠습니까?')){
			...
			f.method    = "post";
			f.target    = "_self";
			f.action    = "/mainContent/deleteSubBanner.do";
			f.submit();
		}
	}
