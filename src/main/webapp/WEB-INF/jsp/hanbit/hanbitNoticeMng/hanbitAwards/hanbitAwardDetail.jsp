...
<form id="frm" name="frm" method="post" enctype="multipart/form-data">
	...
	<div class="cont_block">
		...
		<c:forEach items="${fileList}" var="fileList" varStatus="status">
			<img alt="" style="width: calc(1060px / 3.5); height: 200px;" src="/hanbit/hanbitNoticeMng/habitNoticeImage.do?attachSeq=<c:out value="${fileList.attachSeq}" />&fileSeq=<c:out value="${fileList.fileSeq}"/>&no=1" />
		</c:forEach>
		</div>
		<br/><br/>
...
<script type="text/javascript">
	...
	function goReg() {
		$('#frm').attr('action', '/hanbit/hanbitNoticeMng/hanbitAwardReg.do');
		$('#frm').submit();
	}
