...
<form id="searchForm" name="searchForm" method="post">
	...
	<div class="cont_block mt-20">
		...
		<div>
			<div id="awardListSheet" class="sheet"></div>
		</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		setSheetHeader_awardList();				// 시상식 목록 헤더
		getAwardList();							// 시상식 목록 조회

	});

	function setSheetHeader_awardList() {	// 시상식 목록 헤더
		...
		var container = $('#awardListSheet')[0];
		createIBSheet2(container, 'awardListSheet', '100%', '100%');
		ibHeader.initSheet('awardListSheet');

		awardListSheet.SetEllipsis(1); 				// 말줄임 표시여부
		awardListSheet.SetSelectionMode(4);
	...
	function awardListSheet_OnRowSearchEnd(row) {
		awardListSheet.SetColFontBold('noticeTitle', 1);
	}

	function getAwardList() {	// 시상식 목록 조회

		var searchParam = $('#searchForm').serializeObject();

		global.ajax({
			type : 'POST'
			, url : "/hanbit/hanbitNoticeMng/selectHanbitAwardList.do"
			...
			, success : function(data){
				...
				awardListSheet.LoadSearchData({Data: (data.resultList || []) });
			}
		});
	}

	function awardListSheet_OnClick(Row, Col, Value, CellX, CellY, CellW, CellH, rowType) {

		if(rowType == 'HeaderRow') {
			return;
		}

		var noticeId = awardListSheet.GetCellValue(Row, 'noticeId');

		if(awardListSheet.ColSaveName(Col) == 'noticeTitle') {	// 상세

			$('#noticeId').val(noticeId);
			$('#searchForm').attr('action', '/hanbit/hanbitNoticeMng/hanbitAwardDetail.do');
			$('#searchForm').submit();
		}
	}
	...
	function goWrite() {
		...
		$('#searchForm').attr('action', '/hanbit/hanbitNoticeMng/hanbitAwardNewReg.do');
		$('#searchForm').submit();
	}
