...
<div class="search">
	<table class="formTable">
		...
		<tbody>
			<tr>
				<th scope="row">아이디 / 성명</th>
				<td>
					<input type="text" id="searchNameId" name="searchNameId" value="${param.searchNameId}" onkeydown="onEnter(doSearch);" class="form_text w100p" title="비고" />
				</td>
				<th scope="row">권한명</th>
				<td>
...
<script type="text/javascript">
	...
	ibHeader.addHeader({Header: '아이디', Type: 'Text', SaveName: 'userId', Width: 40, Align: 'Left', Edit: 0, Cursor: 'Pointer'});
	ibHeader.addHeader({Header: '성명', Type: 'Text', SaveName: 'userNm', Width: 40, Align: 'Center', Edit: 0, Cursor: 'Pointer'});
	ibHeader.addHeader({Header: '개인정보 조회', Type: 'Text', SaveName: 'infoCheckYn', Width: 40, Align: 'Center', Edit: 0});
	ibHeader.addHeader({Header: '권한명', Type: 'Text', SaveName: 'authName', Width: 150, Align: 'Left', Edit: 0});
	...
	function authListSheet_OnSearchEnd(code, msg) {
		if (code != 0) {
    		...
    		// 아이디에 볼드 처리
			authListSheet.SetColFontBold('userId', 1);
			// 성명에 볼드 처리
			authListSheet.SetColFontBold('userNm', 1);
	...
	function authListSheet_OnClick(row, col, value, cellX, cellY, cellW, cellH) {
		if (row > 0) {
			if (
				authListSheet.ColSaveName(col) == 'userId'
				|| authListSheet.ColSaveName(col) == 'userNm'
			) {
				var userId = authListSheet.GetCellValue(row, 'userId');
	...
	function getList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/authority/grant/selectList.do" />'
	...
	function goWrite(userId) {
		f.action = '<c:url value="/authority/grant/write.do" />';
	...
	function doDelete() {		
		...
		if (saveJson.data.length) {
			if (confirm('삭제 하시겠습니까?')) {
				...
				global.ajax({
					type : 'POST'
					, url : '<c:url value="/authority/grant/deleteList.do" />'
					...
					, spinner : true
					, success : function(data){
						location.href = '<c:url value="/authority/grant/list.do" />';
					}
				});
			}
		} else {
			alert('삭제할 권한 부여 정보를 선택해 주세요.');
		}
	}
</script>