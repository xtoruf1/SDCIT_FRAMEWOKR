...
<input type="hidden" id="foreignVisaId" name="foreignVisaId" value="" />
...
<script type="text/javascript">
	...
	ibHeader.addHeader({Header: 'foreignVisaId',	Type: 'Text', 	SaveName: 'foreignVisaId', 	Hidden: true});
	...
	ibHeader.addHeader({Header: '추천 외국인',			Type: 'Text', 	SaveName: 'recommendNm', 	Width: 140, Align: 'Center'});
	ibHeader.addHeader({Header: '국적',			Type: 'Text', 	SaveName: 'ctrNm', 	Width: 100, Align: 'Center'});
	ibHeader.addHeader({Header: '상태',			Type: 'Text', 	SaveName: 'statusNm', 		Width: 100, Align: 'Center'});
	ibHeader.addHeader({Header: '출력여부',		Type: 'Text', 	SaveName: 'printYn', 		Width: 60, Align: 'Center'});
	...
	$(document).ready(function(){
		...
		createIBSheet2(container, 'foreignVisaSheet', '100%', '100%');
		ibHeader.initSheet('foreignVisaSheet');
		foreignVisaSheet.SetSelectionMode(4);
	...
	//시트가 조회된 후 실행
	function foreignVisaSheet_OnSearchEnd(code, msg) {
		if (code != 0) {
    		console.log('foreignVisaSheet_OnSearchEnd : ', msg);
    	} else {
    		// 업체명에 볼드 처리
			foreignVisaSheet.SetColFontBold('companyName', 1);
    	}
    }

	//시트 조회 완료 후 하나하나의 행마다 실행
	function foreignVisaSheet_OnRowSearchEnd(row) {
		// 편집할 수 없는 컬럼 색 변경(시트에 편집 여부가 동일하지 않고 혼재 되어 있을시)
		notEditableCellColor('foreignVisaSheet', row);
	}

	//시트 클릭 이벤트
	function foreignVisaSheet_OnClick(row, col, value, cellX, cellY, cellW, cellH) {
		if (row > 0) {
			if (foreignVisaSheet.ColSaveName(col) == 'companyName') {
				var foreignVisaId = foreignVisaSheet.GetCellValue(row, 'foreignVisaId');

				goView(foreignVisaId);
		    }
		}
	}
	...
	function getList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/issue/recommend/selectForeignVisaList.do" />'
			, data : $('#frm').serialize()
			, dataType : 'json'
			, async : true
			, spinner : true
			, success : function(data){
				...
				foreignVisaSheet.LoadSearchData({Data: data.resultList});
			}
		});
	}
	...
	//상세조회 화면
	function goView(foreignVisaId) {
		f.action = '<c:url value="/issue/recommend/foreignVisaDetail.do" />';
		f.foreignVisaId.value = foreignVisaId;
		f.target = '_self';
		f.submit();
	}
