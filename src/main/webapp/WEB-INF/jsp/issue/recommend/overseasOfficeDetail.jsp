...
<input type="hidden" name="overBranchId" id="overBranchId" value="<c:out value="${result.overBranchId}"/>"/>
<input type="hidden" name="certMngId" id="certMngId" value="<c:out value="${result.certMngId}"/>"/>

<div class="location compact">
	<jsp:include page="/WEB-INF/jsp/layouts/navigation.jsp" flush="true" />
	<div class="ml-auto">
		<button type="button" id="btnAgreement" class="btn_sm btn_primary btn_modify_auth" onclick="callAgreementPopup();">동의서</button>
...
<div class="cont_block">
	...
	<table class="formTable">
		...
		<tr>
			<th>회사명</th>
			<td><p class="korTag"><c:out value="${result.companyName}"/></p><p class="engTag hidden"><c:out value="${result.companyNameEng}"/></p></td>
			<th>무역업고유번호</th>
			<td><c:out value="${result.tradeNo}"/></td>
		</tr>
		<tr>
			<th>대표자</th>
			<td><p class="korTag"><c:out value="${result.ceoName}"/></p><p class="engTag hidden"><c:out value="${result.ceoNameEng}"/></p></td>
			<th>생년월일</th>
			<td><c:out value="${result.birthDate}"/></td>
		</tr>
		<tr>
			<th>소재지</th>
			<td colspan="3"><p class="korTag"><c:out value="${result.address}"/></p><p class="engTag hidden"><c:out value="${result.addressEng}"/></p></td>
		</tr>
		<tr>
			<th>담당자 명</th>
			<td><p class="korTag"><c:out value="${result.chargeName}"/></p><p class="engTag hidden"><c:out value="${result.chargeNameEng}"/></p></td>
			<th>담당자 부서</th>
			<td><p class="korTag"><c:out value="${result.chargeDept}"/></p><p class="engTag hidden"><c:out value="${result.chargeDeptEng}"/></p></td>
		</tr>
		<tr>
			<th>담당자 직위</th>
			<td><p class="korTag"><c:out value="${result.chargeName}"/></p><p class="engTag hidden"><c:out value="${result.chargeNameEng}"/></td>
			<th>담당자 전화</th>
		...
		<tr>
			...
			<th>구분</th>
			<td><c:out value="${result.branchTypeNm}"/></td>
		</tr>
		<tr>
			<th>설치지역</th>
			<td colspan="3"><p class="korTag"><c:out value="${result.installArea}"/></p><p class="engTag hidden"><c:out value="${result.installAreaEng}"/></p></td>
		</tr>
		<tr>
			<th>신청언어</th>
			<td>
				<c:if test="${result.docLanguage eq '01'}">국문</c:if>
				<c:if test="${result.docLanguage eq '02'}">
					영문&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<button type="button" id="showKor" class="btn_sm btn_primary" onclick="doShowKor();">국문</button>
					<button type="button" id="showEng" class="btn_sm btn_secondary" onclick="doShowEng();">영문</button>
				</c:if>
			</td>
			<th>신청일시</th>
...
<script type="text/javascript">
	...
	function goList() {
		...
		$('#frm').attr('action', '/issue/recommend/overseasOfficeList.do');
		$('#frm').submit();
	...
	function callApplyPopup(){
		...
		global.openLayerPopup({
			// 레이어 팝업 URL
			popupUrl : '/issue/recommend/popup/overseasOfficeApply.do'
			, params : { overBranchId : $('#overBranchId').val()
	...
	function callRejectPopup(){
		global.openLayerPopup({
			// 레이어 팝업 URL
			popupUrl : '/issue/recommend/popup/overseasOfficeReject.do'
			, params : { overBranchId : $('#overBranchId').val()
						,certMngId : $('#certMngId').val()
				}
			, callbackFunction : function(resultObj){
	...
	function doResetPrint() {
		...
		global.ajax({
			url : '<c:url value="/issue/recommend/resetPrintOverseasOffice.do" />'
			, dataType : 'json'
			, type : 'POST'
			, data : {
	...
	function callAgreementPopup() {
		...
		global.openLayerPopup({
			popupUrl : '/issue/recommend/popup/overseasOfficeAgreement.do'
			, params : {overBranchId : $('#overBranchId').val()}
		});
	}
