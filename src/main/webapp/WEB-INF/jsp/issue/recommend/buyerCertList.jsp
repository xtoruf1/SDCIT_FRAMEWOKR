...
<input type="hidden" id="buyerVisaId" name="buyerVisaId" value="" />
...
<script type="text/javascript">
	...
	ibHeader.addHeader({Header: 'buyerVisaId',	Type: 'Text', 	SaveName: 'buyerVisaId', 	Hidden: true});
	...
	ibHeader.addHeader({Header: '추천바이어',		Type: 'Text', 	SaveName: 'reqName', 		Width: 200, Align: 'Center'});
	ibHeader.addHeader({Header: '국적',			Type: 'Text', 	SaveName: 'nationalNm', 		Width: 100, Align: 'Center'});
	...
	$(document).ready(function(){
		...
		createIBSheet2(container, 'buyerCertSheet', '100%', '100%');
		ibHeader.initSheet('buyerCertSheet');
		buyerCertSheet.SetSelectionMode(4);
	...
	//시트가 조회된 후 실행
	function buyerCertSheet_OnSearchEnd(code, msg) {
		if (code != 0) {
    		console.log('buyerCertSheet_OnSearchEnd : ', msg);
    	} else {
    		// 업체명에 볼드 처리
			buyerCertSheet.SetColFontBold('companyName', 1);
    	}
    }

	//시트 조회 완료 후 하나하나의 행마다 실행
	function buyerCertSheet_OnRowSearchEnd(row) {
		// 편집할 수 없는 컬럼 색 변경(시트에 편집 여부가 동일하지 않고 혼재 되어 있을시)
		notEditableCellColor('buyerCertSheet', row);
	}

	//시트 클릭 이벤트
	function buyerCertSheet_OnClick(row, col, value, cellX, cellY, cellW, cellH) {
		if (row > 0) {
			if (buyerCertSheet.ColSaveName(col) == 'companyName') {
				var buyerVisaId = buyerCertSheet.GetCellValue(row, 'buyerVisaId');
	...
	function getList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/issue/recommend/selectBuyerCertList.do" />'
			, data : $('#frm').serialize()
			, dataType : 'json'
			, async : true
			, spinner : true
			, success : function(data){
				...
				buyerCertSheet.LoadSearchData({Data: data.resultList});
			}
		});
	}
	...
	//상세조회 화면
	function goView(buyerVisaId) {
		f.action = '<c:url value="/issue/recommend/buyerCertDetail.do" />';
		f.buyerVisaId.value = buyerVisaId;
		f.target = '_self';
		f.submit();
	}
