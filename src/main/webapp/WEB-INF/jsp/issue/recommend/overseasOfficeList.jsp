...
<input type="hidden" id="overBranchId" name="overBranchId" value="" />
...
<script type="text/javascript">
	...
	ibHeader.addHeader({Header: 'overBranchId',	Type: 'Text', 	SaveName: 'overBranchId', 	Hidden: true});
	...
	ibHeader.addHeader({Header: '구분',			Type: 'Text', 	SaveName: 'branchTypeNm', 	Width: 100, Align: 'Center'});
	...
	$(document).ready(function(){
		...
		createIBSheet2(container, 'overseasOfficeSheet', '100%', '100%');
		ibHeader.initSheet('overseasOfficeSheet');
		overseasOfficeSheet.SetSelectionMode(4);
	...
	//시트가 조회된 후 실행
	function overseasOfficeSheet_OnSearchEnd(code, msg) {
		if (code != 0) {
    		console.log('overseasOfficeSheet_OnSearchEnd : ', msg);
    	} else {
    		// 업체명에 볼드 처리
			overseasOfficeSheet.SetColFontBold('companyName', 1);
    	}
    }

	//시트 조회 완료 후 하나하나의 행마다 실행
	function overseasOfficeSheet_OnRowSearchEnd(row) {
		// 편집할 수 없는 컬럼 색 변경(시트에 편집 여부가 동일하지 않고 혼재 되어 있을시)
		notEditableCellColor('overseasOfficeSheet', row);
	}

	//시트 클릭 이벤트
	function overseasOfficeSheet_OnClick(row, col, value, cellX, cellY, cellW, cellH) {
		if (row > 0) {
			if (overseasOfficeSheet.ColSaveName(col) == 'companyName') {
				var overBranchId = overseasOfficeSheet.GetCellValue(row, 'overBranchId');
	...
	function getList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/issue/recommend/selectOverseasOfficeList.do" />'
			, data : $('#frm').serialize()
			, dataType : 'json'
			, async : true
			, spinner : true
			, success : function(data){
				...
				overseasOfficeSheet.LoadSearchData({Data: data.resultList});
			}
		});
	}
	...
	//상세조회 화면
	function goView(overBranchId) {
		f.action = '<c:url value="/issue/recommend/overseasOfficeDetail.do" />';
		f.overBranchId.value = overBranchId;
		f.target = '_self';
		f.submit();
	}
