...
<input type="hidden" id="awardClosureId" name="awardClosureId" value="" />
...
<script type="text/javascript">
	...
	ibHeader.addHeader({Header: 'awardClosureId',	Type: 'Text', 	SaveName: 'awardClosureId', Hidden: true});
	...
	$(document).ready(function(){
		...
		createIBSheet2(container, 'closeExportTopSheet', '100%', '100%');
		ibHeader.initSheet('closeExportTopSheet');
		closeExportTopSheet.SetSelectionMode(4);
	...
	//시트가 조회된 후 실행
	function closeExportTopSheet_OnSearchEnd(code, msg) {
		if (code != 0) {
    		console.log('closeExportTopSheet_OnSearchEnd : ', msg);
    	} else {
    		// 업체명에 볼드 처리
			closeExportTopSheet.SetColFontBold('companyName', 1);
    	}
    }

	//시트 조회 완료 후 하나하나의 행마다 실행
	function closeExportTopSheet_OnRowSearchEnd(row) {
		// 편집할 수 없는 컬럼 색 변경(시트에 편집 여부가 동일하지 않고 혼재 되어 있을시)
		notEditableCellColor('closeExportTopSheet', row);
	}

	//시트 클릭 이벤트
	function closeExportTopSheet_OnClick(row, col, value, cellX, cellY, cellW, cellH) {
		if (row > 0) {
			if (closeExportTopSheet.ColSaveName(col) == 'companyName') {
				var awardClosureId = closeExportTopSheet.GetCellValue(row, 'awardClosureId');
				goView(awardClosureId);
		    }
		}
	}
	...
	function getList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/issue/cert/selectCloseExportTopList.do" />'
			, data : $('#frm').serialize()
			, dataType : 'json'
			, async : true
			, spinner : true
			, success : function(data){
				...
				closeExportTopSheet.LoadSearchData({Data: data.resultList});
			}
		});
	}
	...
	//상세조회 화면
	function goView(awardClosureId) {
		f.action = '<c:url value="/issue/cert/closeExportTopDetail.do" />';
		f.awardClosureId.value = awardClosureId;
		f.target = '_self';
		f.submit();
	}
