...
<form name="viewForm" id="viewForm" method="post" onsubmit="return false;">
	...
	<div class="location compact">
		...
		<!-- 네비게이션 -->
		<div class="ml-auto">'
			<button type="button" class="btn_sm btn_primary" onclick="downloadIbSheetExcel(selectListSheet,'한국무역협회 무역지원서비스 은행별통계','');">엑셀 다운</button>
	...
	<div class="tab_header">
		...
		<button class="tab on" onclick="location.href='/svcex/svcexStat/tradeLump5List.do'">은행</button>
		...
		<table class="formTable">
			...
			<tbody>
				<tr>
					<th>은행</th>
					<td colspan="3">
						<select name="searchForeignBank" id="searchForeignBankId" class="form_select">
							<c:forEach var="list" items="${svc003}" varStatus="status">
								<option value="${list.detailcd}"<c:if test="${svcexVO.searchForeignBank == list.detailcd}"> selected</c:if>>${list.detailnm}</option>
							</c:forEach>
						</select>
					</td>
					<th>월/분기별</th>
...
<script type="text/javascript">
	...
	function selectList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/svcex/svcexStat/tradeLump5DetailDataList.do" />'
...
// 				selectListSheet.LoadSearchData({Data: data.resultList}, {Wait: 0});
	...
	function setGrid(data){
		...
		ibHeader.addHeader({Header: "월/분기별", Type: 'Text', SaveName: 'detailnm', Width: 4, Align: 'Center', ColMerge: 0, Edit:false});
		ibHeader.addHeader({Header: svcexVO.lastYear + '년 금액($)', Type: 'AutoSum', SaveName: 'year1', Width: 14, Align: 'right', Edit:false});
		ibHeader.addHeader({Header: svcexVO.searchYear + '년 금액($)', Type: 'AutoSum', SaveName: 'year2', Width: 14, Align: 'right', Edit:false});
