		...
		<table class="formTable">
			...
			<tbody>
				<tr>
					<th>거래형태</th>
					<td colspan="3">
						<select name="searchTradePatternCd" id="searchTradePatternCdId" class="form_select w100p">
							<c:forEach var="list" items="${com019}" varStatus="status">
								<option value="${list.detailcd}"<c:if test="${svcexVO.searchTradePatternCd == list.detailcd}"> selected</c:if>>${list.detailnm}</option>
							</c:forEach>
						</select>
					</td>
					<th>월/분기별</th>
...
<script type="text/javascript">
	...
	function selectList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/svcex/svcexStat/tradeLump4DetailDataList.do" />'
...
// 				selectListSheet.LoadSearchData({Data: data.resultList}, {Wait: 0});
	...
	function setGrid(data){
		...
		ibHeader.addHeader({Header: "월/분기별", Type: 'Text', SaveName: 'detailnm', Width: 5, Align: 'Center', ColMerge: 0, Edit:false});
