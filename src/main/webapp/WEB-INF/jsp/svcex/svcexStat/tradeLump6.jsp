...
<form name="viewForm" id="viewForm" method="post" onsubmit="return false;">
	<input type="hidden" name="searchSido"	id="searchSido"	value=""/>
...
<script type="text/javascript">
	...
	function selectList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/svcex/svcexStat/tradeLump6DataList.do" />'
...
// 				selectListSheet.LoadSearchData({Data: data.resultList}, {Wait: 0});
	...
	function setGrid(data){
		...
		ibHeader.addHeader({Header: '시도|시도', Type: 'Text', SaveName: 'detailnm', Width: 10, Align: 'Center', ColMerge: 0, Hidden: false, Ellipsis: true, Cursor:"Pointer", Edit:false});
		ibHeader.addHeader({Header: svcexVO.lastYear + '년|금액($)', Type: 'AutoSum', SaveName: 'checkAmount1', Width: 30, Align: 'right', Edit:false});
		ibHeader.addHeader({Header: svcexVO.lastYear + '년|업체수', Type: 'AutoSum', SaveName: 'bsNo1', Width: 30, Align: 'right', Edit:false});
		ibHeader.addHeader({Header: svcexVO.searchYear + '년|금액($)', Type: 'AutoSum', SaveName: 'checkAmount2', Width: 30, Align: 'right', Edit:false});
		ibHeader.addHeader({Header: svcexVO.searchYear + '년|업체수', Type: 'AutoSum', SaveName: 'bsNo2', Width: 30, Align: 'right', Edit:false});
	...
	function selectListSheet_OnClick(row, col, value, cellX, cellY, cellW, cellH) {
		if (row > 0) {
			if( selectListSheet.ColSaveName(col) == "detailnm" ) {
				var url = "/svcex/svcexStat/tradeLump6DetailList.do";
				var detailcd = selectListSheet.GetCellValue(row, "detailcd");

				f.searchSido.value = detailcd;
				f.action = url;
				f.target = '_self';
				f.submit();
			}
