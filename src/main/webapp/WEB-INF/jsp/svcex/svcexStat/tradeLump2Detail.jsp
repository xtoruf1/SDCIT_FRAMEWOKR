					...
					<td colspan="5">
						<div class="field_set">
							<span class="form_search">
								<input name="targetCountryCd" id="targetCountryCd" type="hidden" value="<c:out value="${svcexVO.targetCountryCd }"/>">
			               		<input name="targetCountryNm" id="targetCountryNm" type="text" class="form_text" readonly="" value="<c:out value="${svcexVO.targetCountryNm }"/>" desc="국가명">
			               		<button type="button" class="btn_icon btn_search" onclick="viewPopup()" title="국가검색"></button>
		               		</span>
		               		<button type="button" class="ml-8" onclick="mtiClearDate('targetCountryCd');"><img src="<c:url value='/images/admin/btn_clear.png' />" alt="초기화"></button>
	               		</div>
					</td>
				</tr>
				<tr>
...
<script type="text/javascript">
	...
	function selectList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/svcex/svcexStat/tradeLump2DetailDataList.do" />'
...
// 				selectListSheet.LoadSearchData({Data: data.resultList}, {Wait: 0});
	...
	function setGrid(data){
		...
		ibHeader.addHeader({Header: '월/분기별', Type: 'Text', SaveName: 'detailnm', Width: 5, Align: 'Center', Edit:false});
		...
		ibHeader.addHeader({Header: '증감율(%)', Type: 'Text', SaveName: 'per', Width: 15, Align: 'RigRt', Edit:false});
