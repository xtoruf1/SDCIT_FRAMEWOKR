...
<form name="viewForm" id="viewForm" method="post" onsubmit="return false;">
	<input type="hidden" name="searchForeignBank"	id="searchForeignBank"	value=""/>
	<input type="hidden" name="listPage"	id="listPage"	value="<c:out value="/svcex/svcexStat/tradeLump5List.do"/>"/>
...
<script type="text/javascript">
	...
	function selectList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/svcex/svcexStat/tradeLump5DataList.do" />'
...
// 				selectListSheet.LoadSearchData({Data: data.resultList}, {Wait: 0});
	...
	function setGrid(data){
		...
		ibHeader.addHeader({Header: '은행코드', Type: 'Text', SaveName: 'detailcd', Width: 5, Align: 'Center', Edit:false});
		ibHeader.addHeader({Header: '은행', Type: 'Text', SaveName: 'detailnm', Width: 20, Align: 'Left', ColMerge: 0, Hidden: false, Cursor:"Pointer", Edit:false});
...
// 		ibHeader.setHeaderRowMerge([1, 0]);
	...
	function selectListSheet_OnClick(row, col, value, cellX, cellY, cellW, cellH) {
		if (row > 0) {
			var url = "/svcex/svcexStat/tradeLump5DetailList.do";
			var detailcd = selectListSheet.GetCellValue(row, "detailcd");

			f.searchForeignBank.value = detailcd;
			f.action = url;
			f.target = '_self';
			f.submit();
		}
