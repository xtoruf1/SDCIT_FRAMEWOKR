...
<form name="viewForm" id="viewForm" method="post" onsubmit="return false;">
	<input type="hidden" name="searchTradePatternCd"	id="searchTradePatternCd"	value=""/>
	<input type="hidden" name="listPage"	id="listPage"	value="<c:out value="/svcex/svcexStat/tradeLump4List.do"/>"/>
	...
	<div class="location compact">
		...
		<!-- 네비게이션 -->
		<div class="ml-auto">
			<button type="button" class="btn_sm btn_primary" onclick="downloadIbSheetExcel(selectListSheet,'한국무역협회 무역지원서비스 거래형태별통계','');">엑셀 다운</button>
	...
	<div class="tab_header">
		...
		<button class="tab on" onclick="location.href='/svcex/svcexStat/tradeLump4List.do'">거래형태</button>
...
<script type="text/javascript">
	...
	function selectList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/svcex/svcexStat/tradeLump4DataList.do" />'
...
// 				selectListSheet.LoadSearchData({Data: data.resultList}, {Wait: 0});
	...
	function setGrid(data){
		...
		ibHeader.addHeader({Header: '거래코드', Type: 'Text', SaveName: 'detailcd', Width: 5, Align: 'Center', Edit:false});
		ibHeader.addHeader({Header: '거래형태', Type: 'Text', SaveName: 'detailnm', Width: 20, Align: 'left', ColMerge: 0, Hidden: false, Ellipsis: true, Cursor:"Pointer", Edit:false});
		ibHeader.addHeader({Header: svcexVO.lastYear + '년 금액($)', Type: 'AutoSum', SaveName: 'checkAmount1', Width: 15, Align: 'right', RowMerge: 0, Edit:false});
		ibHeader.addHeader({Header: svcexVO.searchYear + '년 금액($)', Type: 'AutoSum', SaveName: 'checkAmount2', Width: 15, Align: 'right', RowMerge: 0, Edit:false});
	...
	function selectListSheet_OnClick(row, col, value, cellX, cellY, cellW, cellH) {
		if (row > 0) {
			if( selectListSheet.ColSaveName(col) == "detailnm" ) {
				var url = "/svcex/svcexStat/tradeLump4DetailList.do";
				var detailcd = selectListSheet.GetCellValue(row, "detailcd");

				f.searchTradePatternCd.value = detailcd;
				f.action = url;
				f.target = '_self';
				f.submit();
			}
