...
<form name="viewForm" id="viewForm" method="post" onsubmit="return false;">
	...
	<input type="hidden" name="listPage"	id="listPage"			value="<c:out value="/svcex/svcexStat/itemStatistics4List.do"/>"/>
	...
	<div class="location compact">
		...
		<!-- 네비게이션 -->
		<div class="ml-auto">
			<button type="button" class="btn_sm btn_primary" onclick="downloadIbSheetExcel(selectListSheet,'한국무역협회 무역지원서비스 품목별수출입(품목의국가월별)','');">엑셀 다운</button>
	...
	<div class="tab_header">
		...
		<button class="tab" onclick="location.href='/svcex/svcexStat/itemStatistics3List.do'">품목의국가</button>
		<button class="tab on" onclick="location.href='/svcex/svcexStat/itemStatistics4List.do'">품목의국가(월)</button>
	</div>
					...
					<td>
						<select name="searchYear" id="searchYearId" class="form_select">
							<c:forEach var="list" items="${yearList}" varStatus="status">
								<option value="${list.searchYear}"<c:if test="${svcexVO.searchYear == list.searchYear}"> selected</c:if>> <c:out value="${list.searchYear}"/> </option>
							</c:forEach>
						</select>
					</td>
					<th>월/분기별</th>
					<td>
						<select name="searchBungiCd" id="searchBungiCdId" class="form_select">
							<c:forEach var="list" items="${svc005}" varStatus="status">
								<option value="${list.detailcd}"<c:if test="${svcexVO.searchBungiCd == list.detailcd}"> selected</c:if>>  <c:out value="${list.detailnm}"/> </option>
							</c:forEach>
						</select>
					</td>
				</tr>
						...
						<div class="field_set">
							<span class="form_search">
								<input name="targetCountryCd" id="targetCountryCd" type="hidden" value="<c:out value="${not empty svcexVO.targetCountryCd ? svcexVO.targetCountryCd : svcexVO.countryCd  }"/>">
			               		<input name="targetCountryNm" id="targetCountryNm" type="text" class="form_text" readonly="" value="<c:out value="${not empty svcexVO.targetCountryNm ? svcexVO.targetCountryNm : svcexVO.countryNm  }"/>" desc="국가명">
					...
					<td>
						<select name="searchExpImpCd" id="searchExpImpCdId" class="form_select">
							<c:forEach var="list" items="${com002}" varStatus="status">
								<option value="${list.detailcd}"<c:if test="${svcexVO.searchExpImpCd == list.detailcd}"> selected</c:if>>  <c:out value="${list.detailnm}"/> </option>
							</c:forEach>
						</select>
					</td>
				</tr>
...
<script type="text/javascript">
	...
	function selectList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/svcex/svcexStat/itemStatistics4DataList.do" />'
