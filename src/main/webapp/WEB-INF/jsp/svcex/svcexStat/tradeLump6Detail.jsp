...
<form name="viewForm" id="viewForm" method="post" onsubmit="return false;">
	...
	<div class="location compact">
		...
		<!-- 네비게이션 -->
		<div class="ml-auto">
			<button type="button" class="btn_sm btn_primary" onclick="downloadIbSheetExcel(selectListSheet,'한국무역협회 무역지원서비스 시도별통계','');">엑셀 다운</button>
	...
	<div class="tab_header">
		...
		<button class="tab on" onclick="location.href='/svcex/svcexStat/tradeLump6List.do'">시도</button>
		...
		<table class="formTable">
			...
			<tbody>
				<tr>
					<th>시도</th>
					<td colspan="3">
						<select name="searchSido" id="searchSidoId" class="form_select">
							<c:forEach var="list" items="${com003}" varStatus="status">
								<option value="${list.detailcd}"<c:if test="${svcexVO.searchSido == list.detailcd}"> selected</c:if>>${list.detailnm}</option>
							</c:forEach>
						</select>
					</td>
					<th>월/분기별</th>
...
<script type="text/javascript">
	...
	function selectList() {
		...
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/svcex/svcexStat/tradeLump6DetailDataList.do" />'
