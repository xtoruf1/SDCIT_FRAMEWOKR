...
<div class="page_tradesos">
	<form name="searchForm" id="searchForm" action ="" method="get" onsubmit="return false;">
		...
		<input type="hidden"  name="qrChk" value="<c:out value='${qrChk }' default='N'/>"/>
		...
		<div class="page_tradeDay_wrapper">
			...
			<div class="tradeDay_header">
				<div class="inner">
					...
					<span class="pc_hidden">무역의 날 기념식 수상내역 확인</span>
				</div>
			</div>
			...
			<div class="tradeDay_body" id="tradeDay_body">
				<div class="tradeDay_tit m_hidden">
					<h2>포상QR확인 : ${resultData.tradeDayTitle}</h2>
				...
				<div class="tradeDay_cont">
					<ul>
						<li>
							<div class="inner">
								<span class="num">${resultData.awardTrgtCnt}</span>
								<span class="txt">수령대상자</span>
							</div>
						</li>
						<li>
							<div class="inner">
								<span class="num blu">${resultData.pickupCnt}</span>
								<span class="txt">수령자</span>
							</div>
						</li>
						<li>
							<div class="inner">
								<span class="num red">${resultData.pickupRate}</span>
								<span class="txt">수령율</span>
							</div>
						</li>
					</ul>
				</div>
				<div class="tradeDay_btn">
					<button type="button" class="btn_tbl_border" onclick="openLayerAwardPrcsnPop();">수동 수상처리</button>
...
<script type="text/javascript">
	...
	*/
	function openLayerEventChngPop(){
		...
		global.openLayerPopup({
			...
			, callbackFunction : function(resultObj) {
				...
				document.searchForm.action = '<c:url value="/tradeDay/clbrt/tradeDayClbrtAwardCnfrm.do" />';
	...
	// 수동수상 처리(팝업)
	function openLayerAwardPrcsnPop(){

		var svrId = $('#svrId').val();

		global.openLayerPopup({
			  popupUrl : '<c:url value="/tradeDay/clbrt/awardPrcsnPopup.do" />'
			, params : {
							svrId : svrId
						}
			, callbackFunction : function(resultObj) {
	 ...
	 * @param qrCode
	 */
	function fnPickupCheck(qrCode, scanner) {

		//데이터 확인
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/tradeDay/clbrt/pickupInfoConfirm.do" />'
			, data : {
					  qrData : qrCode
			         }
			, dataType : 'json'
			...
			, success : function(data){
				...
				if( data.SUCCESS ) {
					// 수령일 시에는 자동 수령 업데이트
					if( data.resultData.pickupYn == "N" || data.resultData.pickupYn == "T" || data.resultData.pickupYn == "C") {
						searchCnd = "Y";  //QR 진입시
					} else {
						searchCnd = "N";
					}
					...
					global.openLayerPopup({
					  ...
					  , params : {
							...
							, awardTypeCd : data.resultData.awardTypeCd
							...
							, pickupYn : data.resultData.pickupYn
							, searchCnd : searchCnd
							, scanner : scanner
						 }
						 , callbackFunction : function(resultObj){
	...
	document.querySelector("#scanner").onkeydown = function (event) {
		...
		if(event.keyCode == 13) {
			fnPickupCheck(inko.ko2en(qrCode), 'Y');
		}
	}
