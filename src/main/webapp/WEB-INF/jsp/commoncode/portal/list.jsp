				...
				<td colspan="3">
					<fieldset class="form_group">
						<div class="group_item">
							<select id="searchClCode" name="searchClCode" class="form_select" style="width: 270px;">
								<option value="" <c:if test="${empty param.searchClCode or param.searchClCode eq ''}">selected="selected"</c:if>>전체</option>
								<c:forEach var="item" items="${classCodeList}" varStatus="status">
									<option value="${item.clCode}" <c:if test="${param.searchClCode eq item.clCode}">selected="selected"</c:if>>${item.clCodeNm}</option>
								</c:forEach>
							</select>
						</div>
						<div class="group_item">
							<select id="searchCondition" name="searchCondition" class="form_select">
								<option value="codeId" <c:if test="${param.searchCondition eq 'codeId'}">selected="selected"</c:if>>코드</option>
								<option value="codeIdNm" <c:if test="${param.searchCondition eq 'codeIdNm'}">selected="selected"</c:if>>코드명</option>
							</select>
						</div>
						<div class="group_item">
...
<script type="text/javascript">
	...
	ibHeader.addHeader({Header: '코드분류', Type: 'Text', SaveName: 'clCodeNm', Width: 80, Align: 'Center'});
	ibHeader.addHeader({Header: '코드', Type: 'Text', SaveName: 'codeId', Width: 50, Align: 'Center', Cursor: 'Pointer'});
	ibHeader.addHeader({Header: '코드명', Type: 'Text', SaveName: 'codeIdNm', Width: 60, Align: 'left'});
	ibHeader.addHeader({Header: '설명', Type: 'Text', SaveName: 'codeIdDc', Width: 120, Align: 'left'});
	ibHeader.addHeader({Header: '사용여부', Type: 'Text', SaveName: 'useAtNm', Width: 40, Align: 'Center'});
	...
	function codeListSheet_OnSearchEnd(code, msg) {
		if (code != 0) {
    		...
    		// 코드에 볼드 처리
			codeListSheet.SetColFontBold('codeId', 1);
    	}
    }

	function codeListSheet_OnClick(row, col, value, cellX, cellY, cellW, cellH) {
		if (row > 0) {
			if (codeListSheet.ColSaveName(col) == 'codeId') {
				var codeId = codeListSheet.GetCellValue(row, 'codeId');
	...
	function getList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/commonCode/portal/selectList.do" />'
	...
	// 등록화면
	function goWrite() {
		f.action = '<c:url value="/commonCode/portal/regist.do" />';
		f.codeId.value = '';
		f.target = '_self';
		f.submit();
	}
	...
	// 조회화면
	function goView(codeId) {
		f.action = '<c:url value="/commonCode/portal/modify.do" />';
		f.codeId.value = codeId;
		f.target = '_self';
		f.submit();
	}
