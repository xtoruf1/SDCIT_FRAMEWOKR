...
<div class="contents">
	...
	<table class="formTable">
		...
		<tr>
			...
			<td>
				<input type="text" id="basecd" name="basecd" value="${resultView.basecd}" class="form_text" maxlength="6" title="공통코드" <c:if test="${not empty resultView.basecd}">readonly="readonly"</c:if> />
			</td>
		</tr>
		<tr>
			<th>공통코드명 <strong class="point">*</strong></th>
			<td>
				<input type="text" id="basenm" name="basenm" value="${resultView.basenm}" class="form_text" maxlength="100" title="공통코드명" />
			</td>
		</tr>
		<tr>
			<th>비고</th>
...
<script type="text/javascript">
	...
	ibHeader.addHeader({Header: '상세코드', Type: 'Text', SaveName: 'detailcd', Width: 120, MinWidth: 120, Align: 'Left', UpdateEdit:false,   InsertEdit:true, EditLen: 10});
	...
	ibHeader.addHeader({Header: 'Lvl', Type: 'Text', SaveName: 'lvl', Width: 100, MinWidth: 100, Align: 'Left', EditLen: 3});
	...
	function getCodeList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/commonCode/service/detailList.do" />'
			, data : {
				basecd : $('#basecd').val()
			}
			, dataType : 'json'
	...
	function doInsert() {
		if (isValid()) {
			if (confirm('저장 하시겠습니까?')) {
				...
				global.ajax({
					type : 'POST'
					, url : '<c:url value="/commonCode/service/insert.do" />'
	...
	function doUpdate() {
		if (isValid()) {
			if (confirm('저장 하시겠습니까?')) {
				...
				global.ajax({
					type : 'POST'
					, url : '<c:url value="/commonCode/service/update.do" />'
					, data : JSON.stringify(ccf)
					, contentType : 'application/json'
					, dataType : 'json'
					, async: false
	...
	function goList() {
		lf.action = '<c:url value="/commonCode/service/list.do" />';
		lf.target = '_self';
		lf.submit();
	}
