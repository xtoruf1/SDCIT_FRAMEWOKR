...
<form action="" id="form" name="form" method="post">
	<div class="cont_block">
		<div class="tit_bar">
			<h3 class="tit_block">엑셀양식 업로드</h3>
			<div class="ml-auto">
	...
	<div class="btn_group mt-20 _center">
		<button type="button" class="btn btn_primary" onclick="doExcelUploadPop();">저장</a>
	</div>
</form>
		...
		if(uploadfile_check(f2)){
			if(confirm('업로드 하시겠습니까?')){
				$("#form").ajaxSubmit({
					...
					type: 'POST',
					dataType:"json",
					url:  '/insurance/saveExcelUpload.do',
					success : function(data) {
						if(data.status == '') {
			            	...
			            	if(data.status == '0000'){
			            		...
			            		closeLayerPopup();
			            	}else if(data.status.split('&')[0] == '9005'){
			            		alert(excelRow + "번째 Row에 " + excelCell + "번째 Cell은 " +'"'+ excelCellName + '"' );
			            		closeLayerPopup();
			            	}else if(data.status.split('&')[0] == '9999'){
