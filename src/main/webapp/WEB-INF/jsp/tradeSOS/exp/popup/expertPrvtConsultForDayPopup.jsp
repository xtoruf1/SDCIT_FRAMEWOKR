...
<c:set var="rsrvDate" value="${fn:substring(param.rsrvDate, 0, 4)}-${fn:substring(param.rsrvDate, 4, 6)}-${fn:substring(param.rsrvDate, 6, 8)}" />
	...
	<div class="popup_body">
		<c:choose>
			<c:when test="${empty resultList}">
				<div style="text-align: center;font-size: 20px;">${rsrvDate}</div>
			</c:when>
			<c:otherwise>
...
<script type="text/javascript">
	function doConfirmConsult(prvtConsultId) {
		if (confirm('상담을 확정합니다. 진행 하시겠습니까?')) {
			...
			<c:forEach var="result" items="${resultList}" varStatus="status">
				var result = {};
				result['expertId'] = '${result.expertId}';
				result['consultTypeNm'] = '${result.consultTypeNm}';
				result['chatId'] = '${result.chatId}';
				result['roomId'] = '${result.roomId}';
				result['consultTypeCd'] = '${result.consultTypeCd}';
				result['statusCd'] = '${result.statusCd}';
				result['statusNm'] = '${result.statusNm}';
				result['reqFileId'] = '${result.reqFileId}';
				result['reqFileSeq'] = '${result.reqFileSeq}';
				result['reqOrgFileNm'] = '${result.reqOrgFileNm}';
				result['rsrvDate'] = '${result.rsrvDate}';
				result['rsrvTime'] = '${result.rsrvTime}';
				result['reqId'] = '${result.reqId}';
				result['reqNm'] = '${result.reqNm}';
				result['companyNm'] = '${result.companyNm}';
				result['expertNm'] = '${result.expertNm}';
				result['consultChannel'] = '${result.consultChannel}';
				result['consultChannelNm'] = '${result.consultChannelNm}';
				result['cancelReason'] = '${result.cancelReason}';
				result['prvtConsultId'] = '${result.prvtConsultId}';
				result['cellPhone'] = '${result.cellPhone}';
				result['endTime'] = '${result.rsrvEndTime}';
				
				prvtId.push(result);
			</c:forEach>
			
			var resultData = {};
			prvtId.forEach(function(data, idx){
				if (data.prvtConsultId == prvtConsultId) {
					resultData = data;
				}
			});
