					...
					<div class="group_datepicker">
						<div class="datepicker_box">
							<span class="form_datepicker">
								<input type="text" id="searchStartDate" name="searchStartDate" value="${param.searchStartDate}" class="txt datepicker" style="width: 110px;" placeholder="작성시작일자" title="작성시작일자" readonly="readonly" />
						...
						<div class="datepicker_box">
							<span class="form_datepicker">
								<input type="text" id="searchEndDate" name="searchEndDate" value="${param.searchEndDate}" class="txt datepicker" style="width: 110px;" placeholder="작성종료일자" title="작성종료일자" readonly="readonly" />
								<img src="<c:url value='/images/icon_calender.png' />" class="ui-datepicker-trigger" alt="캘린더" title="캘린더" />
							</span>
...
<input type="hidden" name="searchReqNm" />
<input type="hidden" name="searchExptTypeCd" />
<input type="hidden" name="searchExptText" />
<input type="hidden" name="searchPblcTitle" />
</form>
<script type="text/javascript">
	...
	function getList() {
		global.ajax({
			url : '<c:url value="/tradeSOS/exp/selectOpenConsultationList.do" />'
			, dataType : 'json'
			, type : 'POST'
	...
	function goView(pblcConsultId) {
		f.action = '<c:url value="/tradeSOS/exp/openConsultationDetail.do" />';
	...
	function doDownloadExcel() {
		var searchStartDate = $('#searchStartDate').val().replace(/-/gi, '');
		var searchEndDate = $('#searchEndDate').val().replace(/-/gi, '');
		
		var f = document.excelForm;
		f.action = '<c:url value="/tradeSOS/exp/openConsultExcelList.do" />';
		f.searchConsultTypeCd.value = $('#searchConsultTypeCd').val();
		f.searchStatusCd.value = $('#searchStatusCd').val();
		f.searchStartDate.value = searchStartDate;
		f.searchEndDate.value = searchEndDate;
		f.searchReqNm.value = $('#searchReqNm').val();
		f.searchExptTypeCd.value = $('#searchExptTypeCd').val();
		f.searchExptText.value = $('#searchExptText').val();
		f.searchPblcTitle.value = $('#searchPblcTitle').val();
		f.target = '_self';
		f.submit();
	}
</script>