...
<div class="location">
	...
	<div class="btnGroup ml-auto">
		<button type="button" class="btn_sm btn_primary" onclick="downloadIbSheetExcel(tblGridSheet,'외국어통번역통계_품목군별통계','');">엑셀 다운</button>
...
<div class="cont_block">
	<div class="tabGroup">
		<div class="tab_header">
			...
			<button class="tab on" onclick="page7();" title="품목군별통계">품목군별통계</button>
		</div>
...
<script type="text/javascript">
	...
	function getList(){
		...
		if(validate){
			global.ajax({
				type : 'POST'
				, url : '<c:url value="/tradeSOS/translation/itemStatisticsAjax.do" />'
	...
	function setGrid(data) {
		...
		ibHeader.addHeader({Type: "Text", 		Header: "종류|종류", SaveName: "gubunNm", 	Align: "Left", Width: 100 , Wrap: 1, ColMerge: 0 });
		ibHeader.addHeader({Type: "Text", 		Header: "종류|종류", SaveName: "serviceNm", 	Align: "Left", Width: 150 , Wrap: 1, ColMerge: 0 });
		for(let i=0; i < searchVO.finalDateList.length; i++) {
			...
			ibHeader.addHeader({Type: "AutoSum", Header: searchVO.finalDateList[i]+"년"+ "|이용금액", 				SaveName: "useMoney"+i, 	Align: "Right", Width: 110 ,Format: "#,##0", ColMerge: 0});
			...
			ibHeader.addHeader({Type: "AutoSum", Header: searchVO.finalDateList[i]+"년"+ "|이용금액\n(협회지원금)", 	SaveName: "tradeMoney"+i, 	Align: "Right", Width: 110 ,Format: "#,##0", ColMerge: 0});
			ibHeader.addHeader({Type: "AutoSum", Header: searchVO.finalDateList[i]+"년"+ "|이용건수\n(협회지원금2)", 	SaveName: "tradeSubCnt"+i, 	Align: "Right", Width: 110 ,Format: "#,##0", ColMerge: 0});
			ibHeader.addHeader({Type: "AutoSum", Header: searchVO.finalDateList[i]+"년"+ "|이용금액\n(협회지원금2)",	SaveName: "tradeSubMoney"+i, 	Align: "Right", Width: 110 ,Format: "#,##0", ColMerge: 0});
		}

		ibHeader.addHeader({Type: "AutoSum", Header: "합계|이용건수", 			    SaveName: "useCntSum",                  	Align: "Right",	Width: 110  , Format: "#,##0", ColMerge: 0});
		ibHeader.addHeader({Type: "AutoSum", Header: "합계|이용금액", 			    SaveName: "useMoneySum", 	                Align: "Right", Width: 110  , Format: "#,##0", ColMerge: 0});
		ibHeader.addHeader({Type: "AutoSum", Header: "합계|이용건수\n(협회지원금)", 	SaveName: "tradeCntSum", 	                Align: "Right", Width: 110  , Format: "#,##0", ColMerge: 0});
		ibHeader.addHeader({Type: "AutoSum", Header: "합계|이용금액\n(협회지원금)", 	SaveName: "tradeSubMoney", 	                Align: "Right", Width: 110  , Format: "#,##0", ColMerge: 0});
		ibHeader.addHeader({Type: "AutoSum", Header: "합계|이용건수\n(협회지원금2)", 	SaveName: "tradeSubCntSum", 	            Align: "Right", Width: 110  , Format: "#,##0", ColMerge: 0});
		ibHeader.addHeader({Type: "AutoSum", Header: "합계|이용금액\n(협회지원금2)",	SaveName: "tradeMoneySubSum", 	            Align: "Right", Width: 110  , Format: "#,##0", ColMerge: 0});
