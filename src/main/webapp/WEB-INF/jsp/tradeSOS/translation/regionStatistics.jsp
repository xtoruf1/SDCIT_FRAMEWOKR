...
<div class="location">
	...
	<div class="btnGroup ml-auto">
		<button type="button" class="btn_sm btn_primary" onclick="downloadIbSheetExcel(tblGridSheet,'외국어통번역통계_지역별통계','');">엑셀 다운</button>
...
<div class="cont_block">
	<div class="tabGroup">
		<div class="tab_header">
			...
			<button class="tab on" onclick="page4();" title="지역별통계">지역별통계</button>
						...
						<table class="formTable">
							...
							<tbody>
								<tr>
									<th scope="row">소재지</th>
									<td>
										<select class="form_select" name="searchCityCd" id="searchCityCd">
											<option value="all" selected="">전체</option>
											<c:forEach items="${regionList}" var="item" varStatus="status">
												<option value="${item.cdId}" <c:if test="${item.cdId eq searchVO.searchCityCd}">selected</c:if>><c:out value="${item.cdNm}"/></option>
											</c:forEach>
										</select>
									</td>
									<th scope="row">일자</th>
...
<script type="text/javascript">
	...
	function getList(){
		...
		if(validate){
			global.ajax({
				type : 'POST'
				, url : '<c:url value="/tradeSOS/translation/regionStatisticsAjax.do" />'
	...
	function setGrid(data) {
		...
		/** 리스트,헤더 옵션 */
		ibHeader.setConfig({AutoFitColWidth: 'colhidden|rowtransaction',  Page: 10, SearchMode: 4, editable: false, ColResize: true, statusColHidden: true, MergeSheet:msPrevColumnMerge + msHeaderOnly, NoFocusMode: 0, Ellipsis: 1});
		ibHeader.setHeaderMode({Sort: 0, ColResize: true, ColMove: true});

		ibHeader.addHeader({Type: "Text", 		Header: "지역|지역", 		SaveName: "cityCdNm", 	Align: "Left", Width: 120, Wrap: 1 });
		for(let i=0; i < searchVO.finalDateList.length; i++) {
			ibHeader.addHeader({Type: "AutoSum", Header: searchVO.finalDateList[i]+"년"+ "|이용건수", 				SaveName: "useCnt"+i, 	Align: "Right",	 Width: 115 ,Format: "#,##0", ColMerge: 0});
			ibHeader.addHeader({Type: "AutoSum", Header: searchVO.finalDateList[i]+"년"+ "|이용금액", 				SaveName: "useSum"+i, 	Align: "Right", Width: 125 ,Format: "#,##0", ColMerge: 0});
			ibHeader.addHeader({Type: "AutoSum", Header: searchVO.finalDateList[i]+"년"+ "|이용건수\n(협회지원금)", 	SaveName: "tradeCnt"+i, 	Align: "Right", Width: 115 ,Format: "#,##0", ColMerge: 0});
			ibHeader.addHeader({Type: "AutoSum", Header: searchVO.finalDateList[i]+"년"+ "|이용금액\n(협회지원금)", 	SaveName: "tradeSum"+i, 	Align: "Right", Width: 125 ,Format: "#,##0", ColMerge: 0});
			ibHeader.addHeader({Type: "AutoSum", Header: searchVO.finalDateList[i]+"년"+ "|이용건수\n(협회지원금2)", 	SaveName: "tradeSubCnt"+i, 	Align: "Right", Width: 115 ,Format: "#,##0", ColMerge: 0});
			ibHeader.addHeader({Type: "AutoSum", Header: searchVO.finalDateList[i]+"년"+ "|이용금액\n(협회지원금2)",	SaveName: "tradeSubSum"+i, 	Align: "Right", Width: 125 ,Format: "#,##0", ColMerge: 0});
		}

		ibHeader.addHeader({Type: "AutoSum", Header: "합계|이용건수", 			    SaveName: "a1",                  	Align: "Right",	Width: 115  , Format: "#,##0", ColMerge: 0});
		ibHeader.addHeader({Type: "AutoSum", Header: "합계|이용금액", 			    SaveName: "b1", 	                Align: "Right", Width: 125  , Format: "#,##0", ColMerge: 0});
		ibHeader.addHeader({Type: "AutoSum", Header: "합계|이용건수\n(협회지원금)", 	SaveName: "c1", 	                Align: "Right", Width: 115  , Format: "#,##0", ColMerge: 0});
		ibHeader.addHeader({Type: "AutoSum", Header: "합계|이용금액\n(협회지원금)", 	SaveName: "d1", 	                Align: "Right", Width: 125  , Format: "#,##0", ColMerge: 0});
		ibHeader.addHeader({Type: "AutoSum", Header: "합계|이용건수\n(협회지원금2)", 	SaveName: "e1", 	                Align: "Right", Width: 115  , Format: "#,##0", ColMerge: 0});
		ibHeader.addHeader({Type: "AutoSum", Header: "합계|이용금액\n(협회지원금2)",	SaveName: "f1", 	                Align: "Right", Width: 125  , Format: "#,##0", ColMerge: 0});
