...
<% pageContext.setAttribute("newLineChar", "\n"); %>
	...
	<script type="text/javascript">
		function fn_list() {
			$("#searchForm").attr("action","/tradeSOS/scene/sceneSuggestList.do");
			$("#searchForm").submit();
		}
	</script>
	...
	<div class="location">
		...
		<div class="ml-auto">
			<button type="button" class="btn_sm btn_primary btn_modify_auth" onclick="fnExpertSearch();">위원 (재)배정</button>
	...
	<div class="page_tradesos">
		<form name="submitForm" id="submitForm" method="post" enctype="multipart/form-data" >
			...
			<input type="hidden" name="reportYn" id="reportYn" value="<c:out value="${resultData.reportYn}"/>"/>
			<input type="hidden" name="expertId" id="expertId" value="<c:out value="${resultData.expertId}"/>"/>
					...
					<tr>
						...
						<td colspan="5">
							<label class="label_form">
								<input type="radio" class="form_radio" name="licenseYn" id="radio1_1" value="Y" <c:out value="${not empty resultData.companyNo ? 'checked' : ''}"/>>
								<span class="label">기업</span>
							</label>
							<label class="label_form">
								<input type="radio" class="form_radio" name="licenseYn" id="radio1_2" value="N" <c:out value="${empty resultData.companyNo ? 'checked' : ''}"/>>
								<span class="label">예비창업자</span>
							</label>
						</td>
					</tr>
					<tr>
						...
						<td>
							<div class="field_set flex align_center">
								<span class="form_search w100p">
									<input class="input form_text w100p" type="text" name="company" id="company" value="<c:out value="${resultData.company}"/>">
						...
						<td>
							<div class="flex align_center">
								<span class="form_search w100p">
									<input type="text" name="tradeNo" id="tradeNo" class="form_text w100p" onkeypress="fnNumber(this)" value="<c:out value="${resultData.tradeNo}"/>">
				...
				<table class="boardwrite formTable">
					...
					<tr>
						...
						<td>
							...
							<input type="text" name="applyGrade" class="form_text w50p" value="<c:out value="${resultData.applyGrade}"/>">
						</td>
						<th scope="row">부서</th>
						<td>
						...
						<td colspan="5">
							...
							<c:forEach var="fileResult" items="${fileList}" varStatus="status">
								<div class="addedFile" id="${fileResult.attachSeqNo}">
									...
									<button type="button" onclick="viewer.showFileContents('${serverName}/tradeSOS/scene/tradeSOSFileDownload.do?attachDocumId=${fileResult.attachDocumId}&attachSeqNo=${fileResult.attachSeqNo}&attachDocumFg=006', '${fileResult.attachDocumNm}', 'membershipexpert_${profile}_${fileResult.attachDocumId}_${fileResult.attachSeqNo}');" class="file_preview btn_tbl_border">
			...
			<c:if test="${resultData.reportYn eq 'Y'}">
				<div class="cont_block">
					<!-- 타이틀 영역 -->
					<div class="tit_bar">
					...
					<table class="boardwrite formTable">
						...
						<tr>
							...
							<td colspan="2">
								<label class="label_form">
									<input type="radio" name="gubun" class="form_radio" id="radio2_1" value="010" <c:out value="${resultData.gubun eq '010' ? 'checked' : ''}"/>>
									<span class="label">현장방문</span>
								</label>
								<label class="label_form">
									<input type="radio" name="gubun" class="form_radio" id="radio2_2" value="020" <c:out value="${resultData.gubun eq '020' || empty resultData.gubun? 'checked' : ''}"/>>
									<span class="label">전화 및 온라인</span>
								</label>
							</td>
							<th scope="row">담당위원</th>
							<td colspan="3"><c:out value="${resultData.expertNm}"/></td>
						</tr>
						<tr>
							<th scope="row">상담내용 <strong class="point">*</strong></th>
							<td colspan="6">
								<textarea  name="content" rows="15" class="form_textarea"><c:out value="${resultData.content}" escapeXml="false"/></textarea>
							</td>
						</tr>
						<tr>
		...
		<div class="cont_block mt-40">
			...
			<!-- .paging-->
			<div id="companyPaging"class="paging ibs"></div>
		</div>
		...
		<form id="companySearchForm" name="companySearchForm">
			<input type="hidden" name="companyNo" value="<c:out value='${resultData.companyNo}'/>"/>
	...
	<script type="text/javascript">
		...
		jQuery(document).ready(function(){
			...
			$("select[name=applyGradeCd]").on("change",function(){
				if ($(this).val() == '999'){
					$("input[name=applyGrade]").attr('disabled', false);
				} else {
					$("input[name=applyGrade]").val("");
					$("input[name=applyGrade]").attr('disabled', true);
		...
		function tblGridSheet_OnSearchEnd(code, msg) {
			if (code != 0) {
				...
				// 요약 볼드 처리
				tblGridSheet.SetColFontBold('ibSheetOption1', 1);
				// 상세 볼드 처리
				tblGridSheet.SetColFontBold('ibSheetOption2', 1);
			}
		}
		...
		function tblGridSheet_OnClick(Row, Col, Value) {
			if (Row > 0){
				...
				if(tblGridSheet.ColSaveName(Col) == "ibSheetOption1") {
					...
					window.open("/tradeSOS/scene/sceneSuggestListDetail.do?no="+no);
				}
			}
		};

		// 무역현장 컨설팅 요약 내역 Popup
		...
		function selectArr(){
			var saveData =  getSaveJsonGrid("itemStepSheet");
			...
			if (saveData[0].mtiUnit < 2){
				alert("2레벨 이상 선택 가능 합니다.");
				return;
			}
			for(var i=0; i < saveData.length; i++){
				if (i == 0){
					mtiCode = saveData[i].mtiCode;
					mtiName = saveData[i].mtiNameKor;
				}else{
					mtiCode += ","+saveData[i].mtiCode;
					mtiName += ","+saveData[i].mtiNameKor;
				}
		...
		function selectAllArr(){
			var saveData =  getSaveJsonGrid("itemKeySheet");
			var totalRow = $('#mtiCode_list option').length + saveData.length;
			var appendHtml = '';
		...
		function fnSubmit(){
			...
			if (fc_chk_byte2(form.companyAddr, 100, "회사주소는") == false) {
				return;
			}
			...
			if ($("input[name=applyCell]").val().trim() == ""){
				alert("휴대폰을 입력해주세요.");
				$("input[name=applyCell]").focus();
				return;
			}
			...
			if (confirm('수정 하시겠습니까?')) {
				if (submitFlag){
					submitFlag = false;
					$('input[name=eventSdcit]').val("Update"); // 분기

					global.ajaxSubmit($('#submitForm'), {
						type: 'POST'
						, url: '<c:url value="/tradeSOS/scene/sceneSuggestDetailProc.do" />'
		...
		// 담당위원 재배정 Popup
		var expertSubmitFlag = true;
		function fnExpertSearch() {
			global.openLayerPopup({
				popupUrl : '<c:url value="/tradeSOS/popup/expertSearchLayer.do" />'
				, callbackFunction : function(resultObj){
					// 컨설턴트 설정
					$("#expertForm input[name=expertId]").val(resultObj.expertId);
					fnExpertSearchResult();
				}
			});
		}
		...
		//담당위원 재배정
		var expertSubmitFlag = true;
		function fnExpertSearchResult(){

			if (expertSubmitFlag){
				$('input[name=eventSdcit]').val("expertUpdate"); // 분기
				global.ajax({
					type : 'POST'
					, url : '<c:url value="/tradeSOS/scene/expertDetailProc.do" />'
					, data : $('#expertForm').serialize()
					, dataType : 'json'
					, async : true
					, spinner : true
					, success : function(data){
						expertSubmitFlag = false;
						alert(data.MESSAGE);
						fn_list();
					}
				});
		...
		function doResult(resultObj) {
			...
			try {
				if ( resultObj.getRequestEvent() == "expertUpdate" ) {
				...
				}else if (resultObj.getRequestEvent() == "Devare" ){
					if( resultObj.getSuccess() == true) {
						alert(resultObj.getMessage());
						fn_list();
					}
