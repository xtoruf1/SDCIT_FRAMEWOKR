...
<script type="text/javascript">
	...
	function getList(pageIndex){
		...
		global.ajax({
			 ...
			 type : 'POST'
			, url : '<c:url value="/tradeSOS/scene/sceneSuggestListAjax.do" />'
	...
	function fn_detail( no) {
		...
		f.action = '<c:url value="/tradeSOS/scene/sceneSuggestListDetail.do" />';
		f.target = '_self';
		f.submit();
	}
	...
	function getExcelList(){
		...
		if(totalCount < 5000) { //최대 5,000건 다운로드 가능
			...
			f = document.searchForm;
			f.action = '<c:url value="/tradeSOS/scene/sceneSuggestExcelDown.do" />';
			f.method = 'POST'
			f.target = '_self';
			f.submit();
		} else {
