				...
				<tr>
					...
					<td colspan="5">
						<label class="label_form">
							<input type="radio" name="licenseYn" id="radio1_1" class="form_radio" value="Y" <c:if test="${ not empty resultData.companyNo}">checked</c:if>> <span for="radio1_1" class="label">기업</span>
						</label>
						<label class="label_form">
			...
			<colgroup>
				...
				<tbody>
					...
					<tr>
						...
						<td colspan="5">
							<input type="text"  name="title" class="form_text w100p" value="<c:out value="${resultData.title}"/>">
						</td>
					</tr>
					<tr>
						<th scope="row">신청내용 <strong class="point">*</strong></th>
						<td colspan="5">
							<textarea  name="regContent" rows="3" class="form_textarea"><c:out value="${resultData.regContent}"/></textarea>
						</td>
					</tr>
		...
		<table class="formTable">
			...
			<tbody>
				<tr>
					...
					<td>
						<label class="label_form">
							<input type="radio" name="gubun" id="radio2_1" class="form_radio" value="010" <c:out value="${resultData.gubun eq '010' ? 'checked' : ''}"/>> <span for="radio2_1" class="label">현장방문</span>
						</label>
						<label class="label_form">
				...
				<tr>
					...
					<td colspan=3">
						<textarea  name="content" rows="20" class="form_textarea"><c:out value="${resultData.content}"/></textarea>
					</td>
				</tr>
				<tr>
					<th scope="row">규제 접수 내용</th>
					<td colspan=3">
						<textarea  name="content2" rows="2" class="form_textarea"><c:out value="${resultData.content2}"/></textarea>
					</td>
				</tr>
			</tbody>
		</table><!-- // 컨설팅 내역 등록 테이블-->
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#selectEmail").on("change",function(){
			$("input[name=apply_email2]").val($(this).val());
		});
		...
		setMitCode('<c:out value="${fn:substring(resultData.item, 0, 1)}"/>','<c:out value="${fn:substring(resultData.item, 0, 2)}"/>');
