...
<script type="text/javascript">
	...
	function fn_list() {
		document.frm.action = "/system/tradeSOS/problem/regulationFreeSuggestList.do";
		document.frm.submit();
	}
	...
	function openLayer(IdName){
		...
		// 팝업 안에 에디터가 있는경우 2021-09-29
		if(openPop.find("#popEditor").length == 1) {
			$("#popEditor").html('<textarea id="req_comp_ans" name="req_comp_ans" rows="3" style="width: 100%;height: 200px" ></textarea>');
			popEditor();
		}
	}
	...
	function popEditor() {
		...
		nhn.husky.EZCreator.createInIFrame({
			oAppRef: oEditors,
			elPlaceHolder: "req_comp_ans",
			sSkinURI: "/smartEditor/SmartEditor2SkinPopup.html",
			htParams : {
			...
			fOnAppLoad : function(){
				...
				temp += "붙임1.";
				//예제 코드
				oEditors.getById["req_comp_ans"].exec("PASTE_HTML", [temp]);
			},
	...
	function fn_print() {
		...
		strUrl = '/tradeSOS/problem/regulationFreeSuggestPrintPop.do';
		strUrl += "?sosSeq="+$('#sosSeq').val();

		window.open(strUrl, "regulationFreeSuggestPrintPop", 'left='+left+',top='+top+',width='+nWidth+',height='+nHeight+',toolbar=no,menubar=no,status=no,scrollbars=yes,resizable=no');
	}
...
<!-- 컨설턴트/전문가 관리 - 수출입 규제 건의 현황 상세 -->
<div style="width: 1170px;height: 750px;" class="fixed_pop_tit">
	<div class="flex popup_top">
		<h2 class="popup_title">수출입 규제 건의 신청 정보</h2>
		<div class="ml-auto">
			<button type="button" class="btn_sm btn_primary" onclick="fn_print();">보고자료출력</button>
			<!-- <button type="button" class="btn_standard st2" onclick="fnMoveFta()">FTA 이관</button> -->
			<!-- <button type="button" class="btn_standard st2" onclick="openLayer('transferPop')">무역실무 이관</button> -->
		</div>
		<c:if test="${edit}">
			<div class="ml-15">
				<button type="button" class="btn_sm btn_primary btn_modify_auth" onclick="fnSubmit();">저장</button>
				<button type="button" class="btn_sm btn_secondary btn_modify_auth" onclick="fnDelete();">삭제</button>
			</div>
		</c:if>
		<div class="ml-15">
			<c:if test="${resultData.proState eq 'FY' && resultData.openYn eq 'Y'}">
				<button type="button" class="btn_sm btn_primary btn_modify_auth" onclick="fnOpen();">비공개</button>
			</c:if>
			<c:if test="${resultData.proState eq 'FY' && resultData.openYn eq 'N'}">
				<button type="button" class="btn_sm btn_primary btn_modify_auth" onclick="fnOpen();">공개</button>
			</c:if>
	...
	<div class="popup_body">
		...
		<!-- <h4 class="para_sub_title">수출입 규제 건의 신청 정보</h4> -->
		<form name="adminForm" id="adminForm" action ="" method="post">
			<input type="hidden" id="sosSeq" name="sosSeq" value="<c:out value="${resultData.sosSeq}"/>"/>
			<input type="hidden" name="proState" value="<c:out value="${resultData.proState}"/>"/>
				...
				<table class="formTable">
					...
					<tr>
							...
							<c:choose>
								<c:when test="${edit}">
										<span class="form_search w100p">
											<input class="form_text w100p" id="compnyNm" name="compnyNm" type="text" maxlength="50" value="<c:out value="${resultData.compnyNm}"/>"/>
						...
						<td>
							<c:choose>
								<c:when test="${edit}">
										<span class="form_search w100p">
											<input class="form_text w100p" name="tradeNum" id="tradeNum" type="text" maxlength="10" value="<c:out value="${resultData.tradeNum}"/>" />
					...
					<tr>
						...
						<td>
							<c:choose>
								<c:when test="${edit}">
									<input type="text" id="presidentKor" name="presidentKor" class="form_text w100p" maxlength="30" value="<c:out value="${resultData.presidentKor}"/>">
								</c:when>
								<c:otherwise>
						...
						<td>
							<c:choose>
								<c:when test="${edit}">
										<span class="form_search w100p">
											<input class="form_text w100p" name="regNo" id="regNo" type="text" maxlength="20" value="<c:out value="${resultData.regNo}"/>"/>
											<button type="button" class="btnSchOpt find btn_icon btn_search" onclick="openTradePopup(2);"></button>
										</span>
								</c:when>
								<c:otherwise>
					...
					<tr>
						...
						<td>
							<input type="text" name="compPhone"  class="form_text w100p" maxlength="20" value="<c:out value="${resultData.compPhone}"/>">
							<%--<c:choose>
								<c:when test="${edit}">
									<div class="flex align_center">
									<select name="compPhone1" class="form_select">
									...
									<input type="text" name="compPhone2" id="compPhone2" maxlength="8" class="form_text w100p ml-8" value="<c:out value="${fn:split(resultData.compPhone,'-')[1]}"/>" maxlength="4">
									<input type="text" name="compPhone3" id="compPhone3" maxlength="8"class="form_text w100p ml-8" value="<c:out value="${fn:split(resultData.compPhone,'-')[2]}"/>" maxlength="4">
									</div>
								</c:when>
								<c:otherwise>
						...
						<th scope="row">FAX</th>
						<td>
							<input type="text" name="compFax" class="form_text w100p" maxlength="20" value="<c:out value="${resultData.compFax}"/>">
							<%--<c:choose>
								<c:when test="${edit}">
										<div class="flex align_center">
											<select name="compFax1" class="form_select">
												<option value="">선택</option>
												<c:forEach var="data" items="${code130}" varStatus="status">
													<option value="${data.cdId}" <c:out value="${fn:split(resultData.compFax,'-')[0] == data.cdId ? 'selected' : ''}"/>>${data.cdId}</option>
												</c:forEach>
											</select>
											<input type="text" name="compFax2" id="compFax2" maxlength="8" class="form_text w100p ml-8" value="<c:out value="${fn:split(resultData.compFax,'-')[1]}"/>" maxlength="4" >
											<input type="text" name="compFax3" id="compFax3" maxlength="8" class="form_text w100p ml-8" value="<c:out value="${fn:split(resultData.compFax,'-')[2]}"/>" maxlength="4" >
										</div>
								</c:when>
								<c:otherwise>
									<c:out value="${resultData.compPhone}"/>
					...
					<tr>
						<th scope="row">지역</th>
						<td>
							<select name="reqArea" class="form_select" <c:out value="${edit ? '' : 'disabled'}"/>>
						...
						<td>
							<c:choose>
								<c:when test="${edit}">
									<input type="text" name="bizItems" maxlength="100" class="form_text w100p" value="<c:out value="${resultData.bizItems}"/>">
								</c:when>
								<c:otherwise>
					...
					<tr>
						...
						<td colspan="3">
							<c:choose>
								<c:when test="${edit}">
									<input type="text" name="compAddr" class="form_text w100p" value="<c:out value="${resultData.compAddr}"/>" maxlength="60">
								</c:when>
								<c:otherwise>
					...
					<tr>
						...
						<td>
							<c:choose>
								<c:when test="${edit}">
									<input type="text" name="reqName" class="form_text w100p" value="<c:out value="${resultData.reqName}"/>">
								</c:when>
								<c:otherwise>
						...
						<td>
							<c:choose>
								<c:when test="${edit}">
									<input type="text" name="reqPostion" class="form_text w100p" value="<c:out value="${resultData.reqPostion}"/>">
								</c:when>
								<c:otherwise>
					...
					<tr>
						...
						<td>
							<c:choose>
								<c:when test="${edit}">
									<input type="text" name="reqDept" class="form_text w100p" value="<c:out value="${resultData.reqDept}"/>">
								</c:when>
								<c:otherwise>
						...
						<td>
							<div class="flex align_center">
								<input type="text" name="reqHp" class="form_text w100p" maxlength="20" value="<c:out value="${resultData.reqHp}"/>">
								<%--							<c:choose>--%>
								<%--								<c:when test="${edit}">--%>
								<%--									<select name="reqHp1" class="form_select">--%>
					...
					<tr>
						...
						<td>
							<div class="flex align_center">
								<input type="text" name="reqPhone" class="form_text w100p" maxlength="20" value="<c:out value="${resultData.reqPhone}"/>">
								<%--		<c:choose>
											<c:when test="${edit}">
												<select name="reqPhone1" class="form_select">
													<option value="">선택</option>
													<c:forEach var="data" items="${code130}" varStatus="status">
						...
						<td>
							<div class="flex align_center">
								<input type="text" name="reqFax" class="form_text w100p" maxlength="20" value="<c:out value="${resultData.reqFax}"/>">
								<%--	<c:choose>
										<c:when test="${edit}">
											<select name="reqFax1" class="form_select">
												<option value="">선택</option>
												<c:forEach var="data" items="${code130}" varStatus="status">
					...
					<tr>
						<th scope="row">이메일</th>
						<td colspan="3">
							<input type="hidden" name="reqEmail" value="${resultData.reqEmail}"/>
							<div class="flex align_center">
								<c:choose>
									<c:when test="${edit}">
					...
					<tr>
						<th scope="row">분야</th>
						<td>
							<select name="reqRgfHight" class="form_select" <c:out value="${edit ? '' : 'disabled'}"/>>
								<option value=""><c:out value="${resultData.reqRgfHight}"/></option>
								<c:forEach var="data" items="${codeRgf}" varStatus="status">
									<option value="${data.cdId}" <c:out value="${resultData.reqRgfHight eq data.cdId ? 'selected' : ''}"/>>${data.cdNm}</option>
								</c:forEach>
							</select>
						</td>
					...
					<tr>
						...
						<td colspan="3">
							<c:out value="${resultData.openYn eq 'Y' ? '공개' : '비공개' }"/>
							<input type="hidden" id="openYn" name="openYn" value="<c:out value="${resultData.openYn}"/>">
						</td>
					</tr>
					<tr>
			...
			<div class="cont_block">
				...
				<div id='processingHistorySheet' class="colPosi"></div>
			</div>
		</form>
	</div> <!-- // .page_tradesos -->
</div>
<script type="text/javascript">
	...
	function f_Init_processingHistory()								//처리이력 목록
	{
		...
		var sheetId = "processingHistorySheet";
		var container = $("#"+sheetId)[0];

		if (typeof processingHistorySheet !== 'undefined' && typeof processingHistorySheet.Index !== 'undefined') {
			processingHistorySheet.DisposeSheet();
		}
	...
	function getFreeProcessList() {
		$.ajax({
			url: '/tradeSOS/problem/regulationFreeProcessResultListAjax.do',
			dataType: 'json',
			type: 'POST',
			data: $('#adminForm').serialize(),
			success: function (data) {
				processingHistorySheet.LoadSearchData({Data: data.resultList});
			},
	...
	function openLayerPerson() {								//담당자 변경 팝업
		global.openLayerPopup({
			...
			, params : {
				pSosSeq : $('#sosSeq').val(),
				ajaxUrl : '/tradeSOS/problem/regulationFreeSuggestDetailProc.do'
			}, callbackFunction : function(resultObj) {
	...
	function fnMoveFta(){
		...
		$.ajax({
			...
			url:"/tradeSOS/problem/regulationFreeSuggestDetailProc.do",
			data:formData,
			processData:false,
			contentType:false,
			async:false,
	...
	//공개/비공개 변경
	function fnOpen() {
		var text = '<c:out value="${resultDate.openYn eq 'Y' ? '비공개' : '공개'}"/>';
		if (confirm(text+'로 변경 하시겠습니까?')) {

			global.ajax({
				type : 'POST'
				, url : '<c:url value="/tradeSOS/problem/regulationFreeSuggestOpenChange.do" />'
				, data : $('#adminForm').serialize()
				, dataType : 'json'
				, async: false
				, spinner : true
	...
	function fnSubmit() {
		...
		if(isStringEmpty(form.reqHp.value)){
	...
	function fnSave() {		//저장,삭제 함수
		var state = $('#eventSdcit').val();
		var text = "";
		if (state == 'Delete'){
			text = '삭제';
		} else {
		...
		if (confirm(text+' 하시겠습니까?')) {
			...
			$.ajax({
				type: "post",
				enctype: 'multipart/form-data',
				url: "/tradeSOS/problem/regulationFreeSuggestProc.do",
				data: formData,
				processData: false,
				contentType: false,
				async: false,
	...
	function doResult(resultObj) {
		...
		try {
			if ( resultObj.getRequestEvent() == "ChgManage" ) {
				if( resultObj.getSuccess() == true) {
					alert(resultObj.getMessage());
					location.reload();
				}
			}else if ( resultObj.getRequestEvent() == "Result" ){
				if( resultObj.getSuccess() == true) {
					alert(resultObj.getMessage());
					location.reload();
				}
			}else if ( resultObj.getRequestEvent() == "MoveConsult" ){
				if( resultObj.getSuccess() == true) {
					alert(resultObj.getMessage());
					window.opener.location.reload();
					window.close();
				}
			}else if ( resultObj.getRequestEvent() == "MoveFta" ){
				if( resultObj.getSuccess() == true) {
					alert(resultObj.getMessage());
					window.opener.location.reload();
					window.close();
			...
			}else if ( resultObj.getRequestEvent() == "Delete" ){
				if( resultObj.getSuccess() == true) {
					alert(resultObj.getMessage());
					window.opener.location.reload();
					window.close();
