...
<div class="page_tradesos">
	...
	<form name="searchForm" id="searchForm" action ="" method="get" onsubmit="return false;">
		...
		<input type="hidden" name="cfrcName" id="cfrcName" value="<c:out value='${searchVO.cfrcName}'/>"/>
		...
		<div class="page_tradeDay_wrapper">
			...
			<div class="tradeDay_header">
				<div class="inner">
					...
					<span class="pc_hidden"><c:out value="${resultData.cfrcName}"/></span>
				</div>
			</div>
			...
			<div class="tradeDay_body" id="tradeDay_body">
				<div class="tradeDay_tit m_hidden">
					<h2><c:out value="${resultData.cfrcName}"/></h2>
					<button type="button" class="btn_sm btn_tbl" onclick="openLayerEventChngPop();">상담회 변경</button>
				</div>
				...
				<div class="tradeDay_cont">
					<ul>
						<li>
							<div class="inner">
								<span class="num"><c:out value="${resultData.cnfCnt}" default='0'/></span>
								<span class="txt">참석대상자</span>
							</div>
						</li>
						<li>
							<div class="inner">
								<span class="num blu"><c:out value="${resultData.attendCnt}" default='0'/></span>
								<span class="txt">참석</span>
							</div>
						</li>
						<li>
							<div class="inner">
								<span class="num red"><c:out value="${resultData.attendRate}" default='0'/>%</span>
...
<script type="text/javascript">
	...
	function fn_cencel() {
		document.searchForm.action = '<c:url value="/technicalConsulting/conferenceMaAtndCnfrm.do" />';
	...
	function openLayerMnlatPrcsnPop(){
		...
		var cfrcName = $('#cfrcName').val();

		global.openLayerPopup({
			  popupUrl : '<c:url value="/technicalConsulting/mnlatPrcsnPopup.do" />'
			, params : {
							devCfrcId : devCfrcId
				          , cfrcName : cfrcName
						}
	...
	function fnAttendCheck(qrCode) {
		...
		var stevCfrcId = $('#devCfrcId').val();

		//데이터 확인
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/technicalConsulting/attendeeInfoConfirm.do" />'
			, data : {
					  qrData : qrCode
				    , devCfrcId : stevCfrcId
			         }
			, dataType : 'json'
			, async : false
			, spinner : true
			, success : function(data){
				...
				if( data.SUCCESS ) {
					...
					global.openLayerPopup({
					  ...
					  , params : {
							  devCfrcId : data.resultData.devCfrcId
						    , applId : data.resultData.applId
						    , companyName : data.resultData.companyName
							, businessNo : data.resultData.businessNo
							, ceoName : data.resultData.ceoName
							, chargeName : data.resultData.chargeName
							, chargePosition : data.resultData.chargePosition
							, chargePhone : data.resultData.chargePhone
