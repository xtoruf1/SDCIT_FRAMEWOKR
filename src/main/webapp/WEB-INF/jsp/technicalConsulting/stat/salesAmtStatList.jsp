...
<div class="location compact">
	...
	<div class="btnGroup ml-auto">
		<button type="button" id="btnExcel" class="btn_sm btn_primary" onclick="downloadIbSheetExcel(statSheet, '평균매출_통계', '');">엑셀 다운</button>
...
<script type="text/javascript">
	...
	ibHeader.addHeader({Header: 'No|No',				Type: 'Text', 		SaveName: 'rn', 			Width: 100, Align: 'Center'});
	ibHeader.addHeader({Header: '개최일|개최일',			Type: 'Text', 		SaveName: 'cfrcStartDate', 	Width: 100, Align: 'Center'});
	ibHeader.addHeader({Header: '상담회명|상담회명',			Type: 'Text', 		SaveName: 'cfrcName', 		Width: 300, Align: 'Left'});
	ibHeader.addHeader({Header: '협력기관|협력기관',			Type: 'Text', 		SaveName: 'partner', 		Width: 150, Align: 'Left'});
	ibHeader.addHeader({Header: '평균매출액|신청',			Type: 'AutoSum', 	SaveName: 'applAvgAmt',		Width: 200, Align: 'Right',	SumType: 'Avg', Format: '#,##0'});
	ibHeader.addHeader({Header: '평균매출액|확정',			Type: 'AutoSum', 	SaveName: 'finishAvgAmt', 	Width: 200, Align: 'Right',	SumType: 'Avg', Format: '#,##0'});
	ibHeader.setConfig({AutoFitColWidth: 'search|resize|init|colhidden|rowtransaction|colresize', Page: 10, SearchMode: 4, DeferredVScroll: 1, SizeMode: 1, MouseHoverMode: 2, NoFocusMode: 0, Ellipsis: 1, MergeSheet: msHeaderOnly+msPrevColumnMerge});
	...
	function getList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/technicalConsulting/stat/selectSalesAmtStatList.do" />'
			, data : $('#frm').serialize()
			, dataType : 'json'
			, async : true
			, spinner : true
