...
<div class="location compact">
	...
	<div class="btnGroup ml-auto">
		<button type="button" id="btnExcel" class="btn_sm btn_primary" onclick="downloadIbSheetExcel(statSheet, '소재지별_통계', '');">엑셀 다운</button>
...
<script type="text/javascript">
	...
	<c:forEach var="list" items="${codeList}" varStatus="status">
		ibHeader.addHeader({Header: '<c:out value="${list.codeNm}"/>',			Type: 'AutoSum', 	SaveName: 'cntCol<c:out value="${list.code}"/>', 		Width: 100, Align: 'Center'});
	</c:forEach>
	...
	function getList() {
		global.ajax({
			type : 'POST'
			, url : '<c:url value="/technicalConsulting/stat/selectAreaStatList.do" />'
			, data : $('#frm').serialize()
			, dataType : 'json'
			, async : true
			, spinner : true
