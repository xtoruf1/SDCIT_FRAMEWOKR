    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/utl/wed/insertSmartEditorImage.do'; 	//upload URL
    ...
    function makeArrayFromString(sResString){
		...
		sResString = decodeURI(sResString);
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/utl/wed/insertSmartEditorImage.do',	//샘플 URL입니다.
 	        sCallback : location.href.replace(/\/[^\/]*$/, '') + '/lib/smarteditor2/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
